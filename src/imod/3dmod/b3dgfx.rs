...
/// `b3dInitializeGL`.
pub fn b3d_initialize_gl(state: &mut B3dGfxState, gl: &mut dyn B3dGfxGl) -> i32 {
    if state.gl_initialized {
        return state.ext_flags;
    }
    let version = gl
        .gl_version()
        ...
        .unwrap_or(0.);
    if version >= 1.5 {
        state.ext_flags |= B3DGLEXT_VERTBUF;
    }
    if version >= 2.0 {
        state.ext_flags |= B3DGLEXT_ANY_SIZE_TEX;
    }
    if version >= 3.1 {
        state.ext_flags |= B3DGLEXT_PRIM_RESTART;
    }
...
pub fn b3d_set_cur_size(state: &mut B3dGfxState, width: i32, height: i32) {
    state.cur_width = width;
    state.cur_height = height;
}
pub fn b3d_get_cur_x_zoom(state: &B3dGfxState) -> f32 {
    state.cur_x_zoom
}
pub fn b3d_set_cur_dev_pix_ratio(state: &mut B3dGfxState, dpr: f32) {
    state.cur_device_pixel_ratio = dpr;
}
pub fn b3d_stipple_next_line(state: &mut B3dGfxState, value: bool) {
    state.stipple_next_line = value;
}
...
pub fn b3d_line_width(state: &B3dGfxState, gl: &mut dyn B3dGfxGl, width: i32, scale_for_dev: bool) {
    gl.line_width(if scale_for_dev {
        state.cur_device_pixel_ratio * width as f32 + 0.001
    } else {
        width as f32
    });
}
pub fn b3d_point_size(state: &B3dGfxState, gl: &mut dyn B3dGfxGl, width: i32, scale_for_dev: bool) {
...
pub fn b3d_draw_cross(state: &B3dGfxState, gl: &mut dyn B3dGfxGl, x: i32, y: i32, size: i32) {
    let s = (state.cur_device_pixel_ratio * size as f32) as i32;
    gl.begin(GL_LINES);
...
pub fn b3d_draw_plus(state: &B3dGfxState, gl: &mut dyn B3dGfxGl, x: i32, y: i32, size: i32) {
    let s = (state.cur_device_pixel_ratio * size as f32) as i32;
    gl.begin(GL_LINES);
    gl.vertex_2i(x - s, y);
    gl.vertex_2i(x + s, y);
...
pub fn b3d_draw_star(state: &B3dGfxState, gl: &mut dyn B3dGfxGl, x: i32, y: i32, size: i32) {
    b3d_draw_plus(state, gl, x, y, size);
    b3d_draw_cross(state, gl, x, y, size)
}
pub fn b3d_draw_triangle(state: &B3dGfxState, gl: &mut dyn B3dGfxGl, x: i32, y: i32, size: i32) {
    let s = (state.cur_device_pixel_ratio * size as f32) as i32;
...
pub fn b3d_draw_filled_triangle(
    state: &B3dGfxState,
    gl: &mut dyn B3dGfxGl,
    x: i32,
    y: i32,
...
pub fn b3d_draw_circle(
    ...
    let radius = if scale_for_dev {
        (state.cur_device_pixel_ratio * radius as f32) as i32
    } else {
        radius
    };
    if radius <= 0 {
...
pub fn b3d_draw_filled_circle(
    ...
    let radius = (state.cur_device_pixel_ratio * radius as f32) as i32;
    if radius <= 0 {
        return;
    }
    gl.line_width(1.0);
...
pub fn b3d_draw_square(state: &B3dGfxState, gl: &mut dyn B3dGfxGl, x: i32, y: i32, size: i32) {
    let s = (state.cur_device_pixel_ratio * size as f32) as i32;
...
pub fn b3d_draw_boxout(
    ...
    if lly > 0 {
        b3d_draw_filled_rectangle(gl, 0, 0, state.cur_width, lly);
    }
    if state.cur_height - ury > 0 {
        b3d_draw_filled_rectangle(gl, 0, ury, state.cur_width, state.cur_height);
    }
    if llx > 0 {
        b3d_draw_filled_rectangle(gl, 0, lly, llx, ury - lly);
    }
    if state.cur_width - urx > 0 {
        b3d_draw_filled_rectangle(gl, urx, lly, state.cur_width, ury - lly);
    }
...
pub fn b3d_draw_arrow(
    ...
    let mut xt = (tip_length as f64 * (angle - std::f64::consts::FRAC_PI_4).cos()).round() as i32;
    let mut yt = (tip_length as f64 * (angle - std::f64::consts::FRAC_PI_4).sin()).round() as i32;
    xt = (state.cur_device_pixel_ratio * xt as f32) as i32;
    yt = (state.cur_device_pixel_ratio * yt as f32) as i32;
    if anti_alias {
        gl.line_smooth(true)
    }
    b3d_line_width(state, gl, thickness, true);
    gl.begin(GL_LINE_STRIP);
...
pub fn b3d_get_new_ci_image(
    ...
    b3d_get_new_ci_image_size(image, depth, state.cur_width, state.cur_height, rgba)
}
...
pub fn b3d_zoom_down_crit(state: &B3dGfxState) -> f32 {
    state.zoom_down_crit
}
...
pub fn b3d_draw_grey_scale_pixels(
    ...
    if !cached {
        ...
        for j in 0..draw_height as usize {
            let sy =
                (yoffset + (j as f64 / yzoom.max(1.)).floor() as i32).clamp(0, ysize - 1) as usize;
            for i in 0..draw_width as usize {
                let sx = (xoffset + (i as f64 / xzoom.max(1.)).floor() as i32).clamp(0, xsize - 1)
                    as usize;
...
pub fn b3d_draw_grey_scale_pixels_hq(
    ...
    b3d_draw_grey_scale_pixels(
        state, gl, data, xsize, ysize, xoffset, yoffset, wx, wy, width, height, image, base, xzoom,
...
pub fn b3d_set_movie_snapping(state: &mut B3dGfxState, snapping: bool) {
    state.movie_snapping = snapping
}
pub fn b3d_set_dpi_scaling(state: &mut B3dGfxState, factor: f32, scale_snap_dpi: bool) {
    state.dpi_scaling = if scale_snap_dpi { factor } else { 1. }
}
pub fn b3d_set_snap_directory(state: &mut B3dGfxState, directory: PathBuf) {
    state.snap_directory = directory
}
pub fn b3d_get_snap_directory(state: &B3dGfxState) -> &Path {
    &state.snap_directory
}
pub fn b3d_get_snapshot_name(
    ...
    loop {
        ...
        *fileno += 1;
        ...
        let candidate = state.snap_directory.join(leaf);
        if !candidate.exists() {
            if first {
                *fileno = 0;
                first = false
...
pub fn b3d_set_snapshot_caption(
    state: &mut B3dGfxState,
    caption_lines: Vec<String>,
    wrap_last_line: bool,
) {
    state.snap_captions = caption_lines;
    state.wrap_last_caption = wrap_last_line
}
...
pub fn snapshot_common(
    ...
    let [x, y, width, height] = limits.unwrap_or([0, 0, state.cur_width, state.cur_height]);
    let mut pixels = vec![
        0;
...
pub fn b3d_snapshot_non_tif(
    ...
    let (width, height, mut pixels) = snapshot_common(state, gl, rgbmode, limits);
    ...
    encoder.write_non_tiff(
        ...
        height,
        &pixels,
        (state.dpi_scaling * dpi as f32 / 0.0254).round() as i32,
    )
}
...
pub fn b3d_snapshot_tif(
    ...
    let (width, height, mut captured) = snapshot_common(state, gl, rgbmode, limits);
    ...
    encoder.write_tiff(
        ...
        height,
        &out,
        (state.dpi_scaling * dpi as f32).round() as i32,
        compression,
        jpeg_quality,
    )
}
...
pub fn b3d_snapshot(
    ...
    if state.snapshot_format == SNAPSHOT_RGB {
        b3d_snapshot_non_tif(
            state, gl, encoder, file, rgbmode, None, None, format, dpi, false,
        )
    } else {
        b3d_snapshot_tif(
            state,
...
mod tests {
    ...
    fn initialize_and_offset_follow_source() {
        ...
        assert_eq!(b3d_initialize_gl(&mut state, &mut gl), 7);
