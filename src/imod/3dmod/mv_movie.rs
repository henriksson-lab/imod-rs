...
impl MvMovieState {
    ...
    pub fn mv_movie_mont_selection(&mut self, n: &mut dyn MvMovieNativeBoundary, mont: i32) {
        self.montage = mont;
        n.sequence_update_enables(mont != 0, self.abort == 0)
    }
    /// Original: `mvMovieQuit`.
    pub fn mv_movie_quit(&mut self, n: &mut dyn MvMovieNativeBoundary) {
        n.close_movie_dialog()
    }
    /// Original: `mvMovieClosing`.
    pub fn mv_movie_closing(&mut self, n: &mut dyn MvMovieNativeBoundary) {
        (
            self.longway,
            self.reverse,
            self.montage,
        ...
        (self.frames, self.mont_frames) = n.get_frame_boxes();
        n.remove_movie_dialog();
        self.movie_dialog_open = false;
        self.abort = 1;
        n.sequence_update_enables(false, false)
    }
    /// Original: `mvMovieStop`.
    pub fn mv_movie_stop(&mut self) {
        self.abort = 1
    }
    /// Original: `mvMovieUpdate`.
    pub fn mv_movie_update(&mut self, n: &mut dyn MvMovieNativeBoundary) {
        n.set_non_tif_label()
    }
    ...
    fn setstep(
        ...
        if index < 3 && (min - max).abs() > 0.05 && self.fullaxis != 0 {
            self.fullaxis = 0;
        }
        if hi != 0 {
            if min < lo as f32 {
    ...
    fn make_montage(&mut self, n: &mut dyn MvMovieNativeBoundary, frames: i32, mut overlap: i32) {
        if frames <= 1 {
            return;
        }
        ...
        if self.abort == 0 {
            n.montage_finish(self.file_format)
        }
        n.montage_end();
        self.abort = 1;
        n.draw();
    ...
    pub fn mv_movie_sequence_dialog(&mut self, n: &mut dyn MvMovieNativeBoundary, state_on: i32) {
        if state_on == 0 {
            if self.sequence_dialog_open {
        ...
        self.sequence_dialog_open = true;
        n.sequence_set_for_four_rows();
        n.sequence_update_enables(self.movie_dialog_open && self.montage == 0, self.abort == 0);
        if self.movie_dialog_open {
            n.set_sequence_open(true);
        }
    }
    /// Original: `mvMovieSequenceClosing`.
    pub fn mv_movie_sequence_closing(&mut self, n: &mut dyn MvMovieNativeBoundary) {
        n.remove_sequence_dialog();
        self.sequence_dialog_open = false;
        if self.movie_dialog_open {
            n.set_sequence_open(false);
        }
    }
    /// Original: `mvMovieGetSegment`.
    pub fn mv_movie_get_segment(&mut self, n: &mut dyn MvMovieNativeBoundary) -> MovieSegment {
        ...
        segment.num_frames = self.frames;
        segment.full_axis = self.fullaxis;
        segment.num_clips = self.start_clips.count.min(self.end_clips.count) as i32;
        segment.clip_flags = self.start_clips.flags as i32;
        for i in 0..segment.num_clips as usize {
            segment.clip_normal[i] = self.start_clips.normal[i];
            segment.start.clip_point[i] = self.start_clips.point[i];
            segment.end.clip_point[i] = self.end_clips.point[i]
        }
        if self.longway != 0 {
        ...
        if self.fullaxis != 0 && segment.label.is_empty() {
    ...
    pub fn mv_movie_segment_array(&mut self) -> &mut Vec<MovieSegment> {
        &mut self.segments
    }

    /// Original: `mvMovieSetStart`.
    pub fn mv_movie_set_start(&mut self, n: &mut dyn MvMovieNativeBoundary) {
        let mut values = std::mem::take(&mut self.start_obj_trans);
        self.set_all_start_or_end(n, IMODV_MOVIE_START_STATE, &mut values);
        self.start_obj_trans = values;
        self.start_clips = n.model_view().clips.clone();
    }
    /// Original: `mvMovieSetEnd`.
    pub fn mv_movie_set_end(&mut self, n: &mut dyn MvMovieNativeBoundary) {
        let mut values = std::mem::take(&mut self.end_obj_trans);
        self.set_all_start_or_end(n, IMODV_MOVIE_END_STATE, &mut values);
        self.end_obj_trans = values;
        self.end_clips = n.model_view().clips.clone();
    }
    ...
    fn make_movie(
        ...
        if !n.standalone() {
            for (j, (lo, hi)) in [(1, dims_x), (1, dims_y), (1, dims_z), (0, 100), (1, dims_z)]
                ...
                .enumerate()
            {
                ip[j] = self.setstep(n, j + 7, steps, lo, hi);
            }
        }
        {
            let v = n.model_view();
            ...
            v.clips = self.start_clips.clone();
        }
        if !n.standalone() {
        ...
        let count = self
            .start_obj_trans
            .len()
            .min(self.end_obj_trans.len())
            .min(3);
        let changed = (0..count).any(|i| self.start_obj_trans[i] != self.end_obj_trans[i]);
        {
            let o = n.model_objects();
            for i in 0..count {
                o[i].trans = self.start_obj_trans[i];
            }
        }
        let has_clip = (0..self.start_clips.count.min(self.end_clips.count) as usize)
            .any(|i| self.start_clips.point[i] != self.end_clips.point[i]);
        if p.iter().all(|x| x.1 == 0.)
            && !has_clip
            && ip.iter().all(|x| x.1 == 0.)
            && !from_sequence
            && !changed
            && self.last_make_nothing < 2
        {
            self.last_make_nothing = n.ask_duplicate_movie();
            if self.last_make_nothing == 0 {
                return 0;
            }
        }
        ...
        self.abort = 0;
        let interval = (1000 / self.trial_fps.max(1)).max(0);
        for frame in 1..=frames {
            if self.saved != 0 {
                n.auto_snapshot(self.file_format)
            } else {
                n.draw();
            ...
            if frame < frames {
                let frac = frame as f32 / (frames - 1).max(1) as f32;
                {
                    ...
                    for pl in 0..self.start_clips.count.min(self.end_clips.count) as usize {
                        v.clips.point[pl] = Ipoint {
                            x: self.start_clips.point[pl].x
                                + frac
                                    * (self.end_clips.point[pl].x - self.start_clips.point[pl].x),
                            y: self.start_clips.point[pl].y
                                + frac
                                    * (self.end_clips.point[pl].y - self.start_clips.point[pl].y),
                            z: self.start_clips.point[pl].z
                                + frac
                                    * (self.end_clips.point[pl].z - self.start_clips.point[pl].z),
                        };
                    }
                }
                if !n.standalone() {
                ...
                let o = n.model_objects();
                for i in 0..count {
                    if self.start_obj_trans[i] != self.end_obj_trans[i] {
                        o[i].trans = (self.start_obj_trans[i] as f32
                            + frac
                                * (self.end_obj_trans[i] as f32 - self.start_obj_trans[i] as f32))
                            .round() as u8;
                    }
                }
            }
    ...
    pub fn mv_movie_set_segment(
        ...
        self.frames = segment.num_frames;
        self.fullaxis = segment.full_axis;
        n.set_frame_boxes(self.frames, self.mont_frames);
        self.set_segment_state(n, segment, &segment.start);
        self.start_clips.count = segment.num_clips.clamp(0, IMOD_CLIPSIZE as i32) as u8;
        self.end_clips.count = self.start_clips.count;
        self.start_clips.flags = segment.clip_flags as u8;
        self.end_clips.flags = self.start_clips.flags;
        for i in 0..self.start_clips.count as usize {
            self.start_clips.normal[i] = segment.clip_normal[i];
            self.end_clips.normal[i] = segment.clip_normal[i];
            self.start_clips.point[i] = segment.start.clip_point[i];
            self.end_clips.point[i] = segment.end.clip_point[i]
        }
        self.longway = if segment.clip_flags & VMOVIE_FLAG_LONG_WAY != 0 {
            1
        } else {
            0
        };
        self.reverse = if segment.clip_flags & VMOVIE_FLAG_REVERSE != 0 {
            1
        } else {
            0
        };
        ...
        let objects = n.model_objects();
        self.start_obj_trans = objects.iter().map(|o| o.trans).collect();
        self.end_obj_trans = self.start_obj_trans.clone();
        for (i, &obj) in segment.trans_change_objs.iter().enumerate() {
            if let Some(x) = self.start_obj_trans.get_mut(obj.max(0) as usize) {
                *x = segment.start.obj_trans[i]
            }
            if let Some(x) = self.end_obj_trans.get_mut(obj.max(0) as usize) {
    ...
    pub fn mv_movie_set_terminus(
        ...
        self.set_segment_state(n, segment, term);
        {
            let v = n.model_view();
            v.rot = term.rotation;
    ...
    pub fn mv_movie_full_axis(&mut self, n: &mut dyn MvMovieNativeBoundary, ixy: i32) {
        self.mv_movie_set_start(n);
        self.mv_movie_set_end(n);
        self.fullaxis = ixy;
    }
    /// Original: `mvMovieMake`.
    pub fn mv_movie_make(&mut self, n: &mut dyn MvMovieNativeBoundary, from_sequence: bool) -> i32 {
        (
            self.longway,
            self.reverse,
            self.montage,
        ...
        if self.abort != 0 {
            if self.montage != 0 {
                self.make_montage(n, self.mont_frames, self.overlap);
                0
            } else {
                self.make_movie(n, self.frames, from_sequence)
            }
        } else {
            0
        }
    ...
    pub fn mv_movie_dialog(&mut self, n: &mut dyn MvMovieNativeBoundary, state_on: i32) {
        if state_on == 0 {
            if self.movie_dialog_open {
                n.close_movie_dialog();
            }
        ...
        n.set_frame_boxes(self.frames, self.mont_frames);
        n.set_sequence_open(self.sequence_dialog_open);
        if self.sequence_dialog_open {
            n.sequence_update_enables(self.montage == 0, false);
        }
    }
}
...
mod tests {
    ...
    fn full_axis_and_segment_round_trip() {
        ...
        n.view.rad = 4.;
        s.mv_movie_full_axis(&mut n, IMODV_MOVIE_FULLAXIS_X);
        let seg = s.mv_movie_get_segment(&mut n);
    ...
    fn movie_steps_display() {
        ...
        n.start[6] = 2.;
        n.end[6] = 4.;
        assert_eq!(s.mv_movie_make(&mut n, false), 0);
        assert_eq!(n.draws, 3);
    ...
    fn movie_and_sequence_dialog_lifecycles_follow_source_branches() {
        ...
        s.mv_movie_sequence_closing(&mut n);
        assert!(!s.sequence_dialog_open);
        assert_eq!(n.sequence_open, vec![false, true, false]);
        s.mv_movie_dialog(&mut n, 0);
    ...
    fn montage_selection_uses_the_source_sequence_enable_predicate() {
        ...
        let mut n = N::default();
        s.mv_movie_mont_selection(&mut n, 1);
        s.mv_movie_mont_selection(&mut n, 0);
