...
use crate::imod::libiimod::iimage::{
    IIERR_BAD_CALL, IIERR_IO_ERROR, IIERR_NOT_FORMAT, IIFILE_MRC, ImodImageFile,
...
pub unsafe extern "C" fn ii_raw_check(in_file: *mut ImodImageFile) -> i32 {
    ...
    let err = unsafe { ii_setup_raw_headers(in_file, (&mut info as *mut RawImageState).cast()) };
    if err != 0 {
        return err;
    }
    unsafe { ii_raw_scan(in_file) }
}

/// C `iiRawScan`.
pub unsafe fn ii_raw_scan(in_file: *mut ImodImageFile) -> i32 {
    if in_file.is_null() {
        return -1;
    }
    ...
    if (unsafe { (*hdr).mode } == MRC_MODE_SHORT || unsafe { (*hdr).mode } == MRC_MODE_USHORT)
        && config.load_int_if_estimate != 0
    {
        config.switch_to_ushort = 1;
        let mut mean = 0.;
        ii_default_min_max_mean(unsafe { (*in_file).type_ }, &mut amin, &mut amax, &mut mean);
        do_scan = false;
    }
    ...
    if do_scan {
        let mut dsize = 0;
        let mut csize = 0;
        mrc_getdcsize(unsafe { (*hdr).mode }, &mut dsize, &mut csize);
        ...
        let (mut yborder, mut xborder, lines_to_scan, skip_lines) = if config.scale_scan_type != 0 {
            let yborder = (border_frac * unsafe { (*hdr).ny } as f32) as i32;
            let mut lines = (subset_bytes / (unsafe { (*hdr).nx } * dsize * csize) as f32) as i32;
            lines = lines.clamp(1, unsafe { (*hdr).ny } - 2 * yborder);
            (
                yborder,
                (border_frac * unsafe { (*hdr).nx } as f32) as i32,
                lines,
                (lines as f32 / subset_frac) as i32,
            )
        } else {
            (0, 0, unsafe { (*hdr).ny }, unsafe { (*hdr).ny })
        };
        li.ymin = yborder;
        li.xmin = 0;
        li.xmax = unsafe { (*hdr).nx } - 1;
        let mut buffer = vec![
            0_u8;
            (unsafe { (*hdr).nx } as usize)
                .saturating_mul(dsize as usize)
                .saturating_mul(csize as usize)
                .saturating_mul(lines_to_scan as usize)
        ];
        let mode_is_real = matches!(
            unsafe { (*hdr).mode },
        ...
        while z < unsafe { (*hdr).nz } {
            ...
            if mode_is_real && !do_mean_sd {
                for iy in 0..lines_to_scan {
                    match unsafe { (*hdr).mode } {
                        MRC_MODE_SHORT => {
                            let data = unsafe {
                                buffer
                                    .as_ptr()
                                    .cast::<i16>()
                                    .add((iy * unsafe { (*hdr).nx } + xborder) as usize)
                            };
                            for ind in xborder..unsafe { (*hdr).nx } - xborder {
                                let value = unsafe { *data.add(ind as usize) } as f32;
                                amin = amin.min(value);
                                amax = amax.max(value);
                            }
                        }
                        ...
                        MRC_MODE_FLOAT => {
                            ...
                            for ind in xborder..unsafe { (*hdr).nx } - xborder {
                                let value = unsafe { *data.add(ind as usize) };
                                amin = amin.min(value);
                                amax = amax.max(value);
                            }
                        }
                ...
                let buf_pixels = (unsafe { (*hdr).nx } - 2 * xborder) as f32 * lines_to_scan as f32;
                let sample_frac = (target_sample / buf_pixels).min(1.);
                let type_smsd = if unsafe { (*hdr).mode } == MRC_MODE_FLOAT {
                    6
                } else if unsafe { (*hdr).mode } == MRC_MODE_SHORT {
                    3
                } else {
                    2
                };
            ...
            z += sec_skip + (li.ymin + rem_skip) / unsafe { (*hdr).ny };
            li.ymin = (li.ymin + rem_skip) % unsafe { (*hdr).ny };
            if li.ymin < yborder {
                li.ymin = yborder;
            }
            if li.ymin + lines_to_scan > unsafe { (*hdr).ny } - yborder {
                if unsafe { (*hdr).ny } - yborder - li.ymin > lines_to_scan / 2 {
                    li.ymin = unsafe { (*hdr).ny } - yborder - lines_to_scan;
                } else {
                    li.ymin = yborder;
                    z += 1;
                }
        ...
        if do_mean_sd {
            unsafe {
                ...
                let fval = (tot_sum_sq - tot_pixels * (*hdr).amean as f64 * (*hdr).amean as f64)
                    / (tot_pixels - 1.);
                ...
                if let Some(mut file) = file {
                    ...
                    if unsafe { mrc_head_read(&mut file, &mut temp) } == 0 {
                        if do_mean_sd {
                            ...
                            if unsafe { (*hdr).amean } < unsafe { (*hdr).amin.min((*hdr).amax) }
                                || (unsafe { (*hdr).amean } == 0.
                                    && (unsafe { (*hdr).amin } == 0.
                                        || unsafe { (*hdr).amax } == 0.))
                            {
        ...
        if !do_mean_sd {
            if config.scale_scan_type != 0
                && (config.switch_to_ushort != 0 || config.raw_image_store == MRC_MODE_USHORT)
            {
                amin -= expand;
                amax += expand;
            }
...
mod tests {
    ...
    fn setters_retain_source_state() {
        ...
        assert_eq!(
            ...
            (RAW_MODE_USHORT, 0, 1, 1)
        );
        assert_eq!((state.amin, state.amax, state.scan_min_max), (-2., 5., 0));
    }
