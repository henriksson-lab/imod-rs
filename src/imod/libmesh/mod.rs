...
static MESH_LIMITS: OnceLock<Mutex<(crate::imod::libimod::imodel::Ipoint, crate::imod::libimod::imodel::Ipoint)>> = OnceLock::new();
...
pub fn find_skeleton_start_point(width: usize, height: usize, axis: &[u8]) -> Option<(usize, usize)> {
    if width < 3 || height < 3 || axis.len() < width.checked_mul(height)? { return None; }
    let on = |x: usize, y: usize| axis[y * width + x] != 0;
    for y in 1..height - 1 { for x in 1..width - 1 {
        if !on(x, y) { continue; }
        let p1 = on(x - 1, y); let p2 = on(x + 1, y); let p3 = on(x, y - 1); let p4 = on(x, y + 1);
        let p5 = on(x - 1, y - 1); let p6 = on(x + 1, y - 1); let p7 = on(x - 1, y + 1); let p8 = on(x + 1, y + 1);
        let sum = [p1,p2,p3,p4,p5,p6,p7,p8].into_iter().filter(|value| *value).count();
        if sum == 1 || (sum == 2 && ((p1&&p5)||(p5&&p3)||(p3&&p6)||(p6&&p2)||(p2&&p8)||(p8&&p4)||(p4&&p7)||(p7&&p1))) { return Some((x,y)); }
    }}
    None
}

fn mesh_limits() -> &'static Mutex<(crate::imod::libimod::imodel::Ipoint, crate::imod::libimod::imodel::Ipoint)> {
    MESH_LIMITS.get_or_init(|| Mutex::new((
        crate::imod::libimod::imodel::Ipoint { x: -1.0e30, y: -1.0e30, z: -1.0e30 },
        crate::imod::libimod::imodel::Ipoint { x: 1.0e30, y: 1.0e30, z: 1.0e30 },
    )))
}

/// Original `imeshSetMinMax` (`libmesh/mkmesh.c`).
pub fn imesh_set_min_max(minimum: crate::imod::libimod::imodel::Ipoint, maximum: crate::imod::libimod::imodel::Ipoint) {
...
pub fn get_next_z(zlist: &[i32], current_z: i32) -> i32 {
    let Some(&last) = zlist.last() else { return current_z.saturating_add(1); };
    for pair in zlist.windows(2) {
        if pair[0] == current_z { return pair[1]; }
    }
...
pub fn chunk_mesh_add_index(
    ...
    if mesh.list.len() >= *max_list {
        *max_list = if mesh.list.is_empty() { CHUNK_SIZE } else { *max_list + CHUNK_SIZE };
...
pub fn chunk_add_triangle(
    ...
    if mesh.list.is_empty() { chunk_mesh_add_index(mesh, IMOD_MESH_BGNPOLY, max_list); }
...
pub fn build_area_matrices(
    ...
    assert!(bottom_size > 0 && top_size > 0, "native mesh area matrices require nonempty contours");
    ...
    let mut i = if bottom_direction < 0 { bottom_size - 1 } else { 0 };
    let mut j = if top_direction < 0 { top_size - 1 } else { 0 };
    for l in 0..top_size {
        let mut next_j = j as i32 + top_direction;
        if next_j == top_size as i32 { next_j = 0; }
        if next_j < 0 { next_j = top_size as i32 - 1; }
        let next_j = next_j as usize;
        for k in 0..bottom_size {
            let mut next_i = i as i32 + bottom_direction;
            if next_i == bottom_size as i32 { next_i = 0; }
            if next_i < 0 { next_i = bottom_size as i32 - 1; }
...
pub fn make_cap_mesh(
    ...
    use crate::imod::libimod::imesh::{
        imod_mesh_add_index, imod_mesh_add_vert, IMOD_MESH_BGNPOLY, IMOD_MESH_END,
        IMOD_MESH_ENDPOLY,
    };
    ...
    for point in 0..contour.pts.len() {
        ...
        if state & state_test != 0 {
            for offset in 1..=3 {
                if istore_generate_items(
                    &mut mesh.store, props, state, mesh.list.len() as i32 + offset, state_test,
                ) != 0 {
                    return None;
                }
            }
        ...
        imod_mesh_add_index(&mut mesh, if mesh_direction != 0 { 0 } else { point as i32 + 1 });
        imod_mesh_add_index(&mut mesh, next as i32 + 1);
        imod_mesh_add_index(&mut mesh, if mesh_direction != 0 { point as i32 + 1 } else { 0 });
...
pub fn contour_cap_mesh(
    ...
    use crate::imod::libimod::icont::{imod_cont_z_direction, IMOD_CONTOUR_CLOCKWISE, IMOD_CONTOUR_COUNTER_CLOCKWISE};
    if contour.pts.is_empty() { return None; }
    ...
    if side > 0 { center.z += 0.5; } else { center.z -= 0.5; }
    let direction = imod_cont_z_direction(Some(contour));
    let mut mesh_direction = i32::from((side < 0 && direction == IMOD_CONTOUR_COUNTER_CLOCKWISE)
        || (side > 0 && direction == IMOD_CONTOUR_CLOCKWISE));
    if inside { mesh_direction = 1 - mesh_direction; }
    make_cap_mesh(contour, &center, mesh_direction, &crate::imod::libimod::istore::DrawProps::default(), 0, 0)
}
...
pub fn make_tube_cont(
    ...
    use crate::imod::libimod::imat::{
        imod_mat_delete, imod_mat_new, imod_mat_rot, imod_mat_scale, imod_mat_transform, B3D_Y,
        B3D_Z,
    };
    use crate::imod::libimod::ipoint::imod_point_append;

    let mut matrix = match imod_mat_new(3) { Some(matrix) => matrix, None => return -1 };
    let mut rotation = match imod_mat_new(3) { Some(matrix) => matrix, None => return -1 };
    ...
    let inverse_scale = crate::imod::libimod::imodel::Ipoint {
        x: 1. / scale.x, y: 1. / scale.y, z: 1. / scale.z,
    };
    imod_mat_scale(&mut matrix, &inverse_scale);
    let source = crate::imod::libimod::imodel::Ipoint { x: 0., y: tube_diameter * 0.5, z: 0. };
    for _ in 0..slices {
...
fn circle_top_and_direction(
    ...
    for point in 0..=contour.pts.len() {
        ...
        if point == top + 1 { reverse = transformed.x > top_x; }
    }
    (reverse, top)
}
...
pub fn join_tube_cont(
    ...
    use crate::imod::libimod::imat::{imod_mat_delete, imod_mat_new, imod_mat_rot, B3D_Y, B3D_Z};
    use crate::imod::libimod::imesh::{IMOD_MESH_BGNPOLY, IMOD_MESH_END, IMOD_MESH_ENDPOLY};
    use crate::imod::libimod::istore::istore_generate_items;

    if first.pts.is_empty() || second.pts.is_empty() { return None; }
    ...
    let second_direction: i32 = if reverse_first == reverse_second { 1 } else { -1 };
    if (first_state | second_state) & (1 << 2) != 0 {
        if first_props.trans != 0 && second_props.trans == 0 { second_props.trans = 1; }
        if second_props.trans != 0 && first_props.trans == 0 { first_props.trans = 1; }
        first_state |= 1 << 2;
        second_state |= 1 << 2;
    }
    ...
    for _ in 0..second_size {
        ...
        second_point = ((second_point as i32 + second_direction).rem_euclid(second_size as i32)) as usize;
    }
    ...
    for point in 0..max_size {
        ...
        let next_first = (((first_size * next) as f64 / max_size as f64 + 0.5).floor() as usize) % first_size;
        let next_second = (((second_size * next) as f64 / max_size as f64 + 0.5).floor() as usize) % second_size;
        if last_first != next_first {
            mesh.list.extend_from_slice(&[first_size as i32 + last_second as i32, last_first as i32, next_first as i32]);
            if generate_items {
                let base = mesh.list.len() as i32 - 3;
                if istore_generate_items(&mut mesh.store, second_props, second_state, base, STATE_TEST) != 0
                    || istore_generate_items(&mut mesh.store, first_props, first_state, base + 1, STATE_TEST) != 0
                    || istore_generate_items(&mut mesh.store, first_props, first_state, base + 2, STATE_TEST) != 0 { return None; }
            }
        }
        if next_second != last_second {
            mesh.list.extend_from_slice(&[first_size as i32 + last_second as i32, next_first as i32, first_size as i32 + next_second as i32]);
            if generate_items {
                let base = mesh.list.len() as i32 - 3;
                if istore_generate_items(&mut mesh.store, second_props, second_state, base, STATE_TEST) != 0
                    || istore_generate_items(&mut mesh.store, first_props, first_state, base + 1, STATE_TEST) != 0
                    || istore_generate_items(&mut mesh.store, second_props, second_state, base + 2, STATE_TEST) != 0 { return None; }
            }
        }
...
pub fn ends_of_whole_gap(
    ...
    assert!(size > 0, "native endsOfWholeGap requires a nonempty contour");
    let mut next = point;
    let mut new_start = point;
    let mut count = 0;
    while count < size / 2 {
        next = if next != 0 { next - 1 } else { size - 1 };
        if istore_point_is_gap(&contour.store, next) == 0 { break; }
        new_start = next;
        count += 1;
    }
    next = (point + 1) % size;
...
pub fn add_connector_if_none(
    ...
    use crate::imod::libimod::istore::{
        istore_connect_number, istore_insert, GEN_STORE_CONNECT, GEN_STORE_ONEPOINT, Istore,
        StoreUnion,
    };
    ...
    for which in 0..2 {
        *maximum_connector += 1;
        ...
        let bottom_index = if which == 0 { bottom_point } else { bottom_next };
        if istore_insert(&mut bottom.store, Istore {
        ...
        } else if which == 0 { top_point } else { top_next };
        if istore_insert(&mut top.store, Istore {
            type_: GEN_STORE_CONNECT,
            flags: GEN_STORE_ONEPOINT,
...
pub fn manage_gaps(
    ...
    use crate::imod::libimod::ipoint::{imod_point3d_scale_distance, imod_point_distance};
    use crate::imod::libimod::istore::{
        istore_count_items, istore_insert, istore_point_is_gap, GEN_STORE_CONNECT, GEN_STORE_GAP,
        GEN_STORE_ONEPOINT, Istore, StoreUnion,
    };
    if bottom.pts.is_empty() || top.pts.is_empty() { return 1; }
    ...
    if closed_object && bottom.flags & ICONT_OPEN != 0
        && istore_point_is_gap(&bottom.store, bottom.pts.len() as i32 - 1) == 0 {
        if istore_insert(&mut bottom.store, Istore {
            type_: GEN_STORE_GAP, flags: GEN_STORE_ONEPOINT,
            index: StoreUnion::from_i(bottom.pts.len() as i32 - 1), value: StoreUnion::default(),
        }) != 0 { return 1; }
        bottom_gaps += 1;
    }
    if closed_object && top.flags & ICONT_OPEN != 0
        && istore_point_is_gap(&top.store, top.pts.len() as i32 - 1) == 0 {
        if istore_insert(&mut top.store, Istore {
            type_: GEN_STORE_GAP, flags: GEN_STORE_ONEPOINT,
            index: StoreUnion::from_i(top.pts.len() as i32 - 1), value: StoreUnion::default(),
        }) != 0 { return 1; }
        top_gaps += 1;
    }
    if bottom_gaps == 0 || top_gaps == 0 { return 0; }
        ...
        for store in &contour.store {
            if store.type_ == GEN_STORE_GAP && store.index.i() >= 0 {
                ...
                if !gaps.contains(&start) {
                    if next < start && (next > 0 || start < contour.pts.len() as i32 - 1) { large_open = Some(gaps.len()); }
                    gaps.push(start);
                }
            }
            if store.type_ == GEN_STORE_CONNECT { maximum = maximum.max(store.value.i()); }
        }
    ...
    if let (Some(bottom_open), Some(top_open)) = (bottom_open, top_open) {
        if add_connector_if_none(bottom, bottom_list[bottom_open], top, top_list[top_open], &mut maximum, direction) != 0 { return 1; }
        end_connected = true;
    ...
    while bottom_left != 0 && top_left != 0 {
        ...
        for (bottom_item, &bottom_start) in bottom_list.iter().enumerate() {
            ...
            for (top_item, &top_start) in top_list.iter().enumerate() {
                ...
                if distance < minimum_distance { minimum_distance = distance; selected_bottom = bottom_item; selected_top = top_item; }
            }
        }
        ...
        let (bottom_next, _) = ends_of_whole_gap(bottom, bottom_start);
        let (top_next, _) = ends_of_whole_gap(top, top_start);
        let maximum_gap = imod_point_distance(&bottom.pts[bottom_start as usize], &bottom.pts[bottom_next as usize])
            .max(imod_point_distance(&top.pts[top_start as usize], &top.pts[top_next as usize]));
        if minimum_distance < maximum_gap || no_gaps {
            if add_connector_if_none(bottom, bottom_start, top, top_start, &mut maximum, direction) != 0 { return 1; }
            if bottom_start == bottom.pts.len() as i32 - 1 || top_start == top.pts.len() as i32 - 1 { end_connected = true; }
        }
    ...
    if closed_object && bottom.flags & ICONT_OPEN != 0 && top.flags & ICONT_OPEN != 0 && !end_connected {
        return add_connector_if_none(bottom, bottom.pts.len() as i32 - 1, top, top.pts.len() as i32 - 1, &mut maximum, direction);
    }
    0
}
...
pub fn make_connectors(
    ...
    use crate::imod::libimod::istore::{
        istore_connect_number, istore_count_items, istore_point_is_gap, GEN_STORE_CONNECT,
        GEN_STORE_NOINDEX,
    };
    if bottom.pts.is_empty() || top.pts.is_empty() { return None; }
    let maximum = istore_count_items(&bottom.store, GEN_STORE_CONNECT, 0)
        .min(istore_count_items(&top.store, GEN_STORE_CONNECT, 0));
    if maximum == 0 { return None; }
    let min_bridge = std::env::var("MIN_BRIDGE_CONNECT_NUM").ok()
        .and_then(|text| text.parse::<i32>().ok()).unwrap_or(0);
    ...
    for store in &bottom.store {
        if store.type_ != GEN_STORE_CONNECT { continue; }
        let number = store.value.i();
        if direction_product == 0 && number < min_bridge { continue; }
        if connectors.iter().any(|connector: &Connector| connector.connect == number) { continue; }
        let Some(top_store) = top.store.iter().find(|candidate|
            candidate.type_ == GEN_STORE_CONNECT && candidate.value.i() == number) else { continue; };
        if !connectors.is_empty() {
            if direction_product == 0 { return None; }
            ...
            if closed_object {
                if connectors.len() > 1
                    && ((direction_product * (middle - last) + top_size) % top_size
                        ...
                        != (direction_product * (start - last) + top_size) % top_size) { continue; }
            } else {
                if connectors.len() == 2 { extraction_open_direction = if last - start > 0 { 1 } else { -1 }; }
                if (middle - last) * extraction_open_direction < 0 { continue; }
            }
        }
        let mut connector = Connector {
            b1: store.index.i(), b2: store.index.i(), t1: top_store.index.i(), t2: top_store.index.i(),
            connect: number, ..Default::default()
        };
        let mut used = false;
        for candidate in &bottom.store {
            if candidate.flags & (GEN_STORE_NOINDEX | 3) != 0 { break; }
            if candidate.type_ == GEN_STORE_CONNECT && candidate.value.i() == number {
                if candidate.index.i() == connector.b1 + 1 { connector.b2 += 1; used = true; }
                if candidate.index.i() == bottom_size - 1 && connector.b1 == 0 && closed_object {
                    connector.b1 = bottom_size - 1; used = true;
                }
            }
        }
        if !used {
            for candidate in &top.store {
                if candidate.flags & (GEN_STORE_NOINDEX | 3) != 0 { break; }
                if candidate.type_ == GEN_STORE_CONNECT && candidate.value.i() == number {
                    if candidate.index.i() == connector.t1 + 1 { connector.t2 += 1; used = true; }
                    if candidate.index.i() == top_size - 1 && connector.t1 == 0 && closed_object {
                        connector.t1 = top_size - 1; used = true;
                    }
                }
            }
        }
    ...
    if direction_product == 0 { return (!connectors.is_empty()).then_some(connectors); }
    ...
    for current in 0..connectors.len() {
        ...
        if !(connector.gap == 0 && (connector.b1 != connector.b2 || connector.t1 != connector.t2)) {
            let other_bottom = |index: i32, connectors: &[Connector]| connectors.iter().enumerate()
                .any(|(item, candidate)| item != current && (candidate.b1 == index || candidate.b2 == index));
            let other_top = |index: i32, connectors: &[Connector]| connectors.iter().enumerate()
                .any(|(item, candidate)| item != current && (candidate.t1 == index || candidate.t2 == index));
            let forward = (connector.b1 + 1) % bottom_size;
            if connector.b1 == connector.b2 && istore_point_is_gap(&bottom.store, connector.b1) != 0
                && !(connector.b1 == bottom_size - 1 && open_object) && !other_bottom(forward, &connectors)
                && ((closed_object && istore_connect_number(&bottom.store, forward) >= 0)
                    || istore_connect_number(&bottom.store, forward) == connector.connect) {
                connector.b2 = forward; connector.gap = 1;
            }
            let backward = (bottom_size + connector.b1 - 1) % bottom_size;
            if connector.b1 == connector.b2 && istore_point_is_gap(&bottom.store, backward) != 0
                && !(backward == bottom_size - 1 && open_object) && !other_bottom(backward, &connectors)
                && ((closed_object && istore_connect_number(&bottom.store, backward) >= 0)
                    || istore_connect_number(&bottom.store, backward) == connector.connect) {
                connector.b1 = backward; connector.gap = 1;
            }
            let forward = (connector.t1 + 1) % top_size;
            if connector.t1 == connector.t2 && istore_point_is_gap(&top.store, connector.t1) != 0
                && !(connector.t1 == top_size - 1 && open_object) && !other_top(forward, &connectors)
                && ((closed_object && istore_connect_number(&top.store, forward) >= 0)
                    || istore_connect_number(&top.store, forward) == connector.connect) {
                connector.t2 = forward; connector.gap = 1;
            }
            let backward = (top_size + connector.t1 - 1) % top_size;
            if connector.t1 == connector.t2 && istore_point_is_gap(&top.store, backward) != 0
                && !(backward == top_size - 1 && open_object) && !other_top(backward, &connectors)
                && ((closed_object && istore_connect_number(&top.store, backward) >= 0)
                    || istore_connect_number(&top.store, backward) == connector.connect) {
                connector.t1 = backward; connector.gap = 1;
            }
        }
        connectors[current] = connector;
    }
    let open_direction = if closed_object { direction_product } else { extraction_open_direction };
    for current in 0..connectors.len() {
        ...
        if current == 0 && open_object {
            if ((connector.b1 > 0 && connector.t1 == 0 && open_direction >= 0)
                ...
                if number >= 0 && number != connector.connect { connector.skip_from_start = 1; connector.skip_index = connector.b1 - 1; }
            }
            if connector.b1 == 0 && connector.t1 > 0 && connector.t2 < top_size - 1 {
                if open_direction >= 0 {
                    let number = istore_connect_number(&top.store, connector.t1 - 1);
                    if number >= 0 && number != connector.connect { connector.skip_from_start = 1; connector.skip_index = connector.t1 - 1; }
                }
                if connector.skip_from_start == 0 && open_direction <= 0 {
                    let number = istore_connect_number(&top.store, connector.t2 + 1);
                    if number >= 0 && number != connector.connect { connector.skip_from_start = 1; connector.skip_index = connector.t2 + 1; }
                }
            }
        }
        if current < connectors.len() - 1 || closed_object {
            let first_number = istore_connect_number(&bottom.store, (connector.b2 + 1) % bottom_size);
            let second_number = istore_connect_number(&bottom.store, (bottom_size + connectors[next].b1 - 1) % bottom_size);
            if first_number >= 0 && second_number >= 0 && first_number != connector.connect
                && first_number != connectors[next].connect && second_number != connector.connect
                && second_number != connectors[next].connect { connector.skip_to_next = 1; }
            if connector.skip_to_next == 0 {
                let first_index = if direction_product > 0 { (connector.t2 + 1) % top_size } else { (top_size + connector.t1 - 1) % top_size };
                let second_index = if direction_product < 0 { (connectors[next].t2 + 1) % top_size } else { (top_size + connectors[next].t1 - 1) % top_size };
            ...
            if ((connector.b2 < bottom_size - 1 && connector.t1 == 0 && open_direction <= 0)
                ...
                if number >= 0 && number != connector.connect { connector.skip_to_end = 1; connector.skip_index = connector.b2 + 1; }
            }
            if connector.b2 == bottom_size - 1 && connector.t1 > 0 && connector.t2 < top_size - 1 {
                if open_direction >= 0 {
                    let number = istore_connect_number(&top.store, connector.t2 + 1);
                    if number >= 0 && number != connector.connect { connector.skip_to_end = 1; connector.skip_index = connector.t2 + 1; }
                }
                if connector.skip_to_end == 0 && open_direction <= 0 {
                    let number = istore_connect_number(&top.store, connector.t1 - 1);
                    if number >= 0 && number != connector.connect { connector.skip_to_end = 1; connector.skip_index = connector.t1 - 1; }
                }
            }
        }
        connectors[current] = connector;
...
pub fn backtrace_cost_path(path: &[u8], bottom_dimension: usize, bottom_length: usize,
                           top_length: usize) -> Vec<CostPathStep> {
    let stride = bottom_dimension + 1;
    assert!(path.len() >= stride * (top_length + 1), "native path matrix is undersized");
...
pub fn mesh_unconnected_contour_pair(
    ...
    let bottom_size = bottom.pts.len();
    let top_size = top.pts.len();
    if bottom_size == 0 || top_size == 0 || start[0] >= bottom_size || start[1] >= top_size { return None; }
    if (!open_object && bottom_size < 3 && top_size < 3) || (open_object && bottom_size < 2 && top_size < 2) { return None; }
    let bottom_dimension = bottom_size - usize::from(open_object);
    let top_dimension = top_size - usize::from(open_object);
    let (up, down) = build_area_matrices(bottom, direction[0], top, direction[1], scale, open_object);
    let bottom_start = if direction[0] < 0 { bottom_size - 1 - start[0] } else { start[0] };
    let top_start = if direction[1] < 0 { top_size - 1 - start[1] } else { start[1] };
    let (_, path) = cost_from_area_matrices(&up, &down, bottom_dimension, top_dimension, bottom_size,
                                             bottom_start, top_start, bottom_dimension, top_dimension, -1.);
    let mut mesh = crate::imod::libimod::imodel::Imesh::default();
    ...
    for index in 0..bottom_size {
        ...
        if bottom_point == bottom_size as i32 { bottom_point = 0; bottom_skip = Some(index); }
        else if bottom_point < 0 { bottom_point = bottom_size as i32 - 1; bottom_skip = Some(index); }
    }
    ...
    for index in 0..top_size {
        ...
        if top_point == top_size as i32 { top_point = 0; top_skip = Some(index); }
        else if top_point < 0 { top_point = top_size as i32 - 1; top_skip = Some(index); }
    }
    ...
    while bottom_count != 0 || top_count != 0 {
        ...
        if path[path_index] != 0 {
            ...
            let top_point = ((start[1] as i32 + direction[1] * top_count as i32).rem_euclid(top_size as i32)) as usize;
            let last_point = ((start[0] as i32 + direction[0] * last_bottom as i32).rem_euclid(bottom_size as i32)) as usize;
            let point = ((start[0] as i32 + direction[0] * bottom_count as i32).rem_euclid(bottom_size as i32)) as usize;
            if !((bottom.flags & ICONT_OPEN != 0 && bottom_skip == Some(last_bottom))
                || istore_point_is_gap(&bottom.store, if direction[0] > 0 { last_point as i32 } else { point as i32 }) != 0
                || outside_mesh_limits(&top.pts[top_point], &bottom.pts[last_point], &bottom.pts[point])) {
                chunk_add_triangle(&mut mesh, (top_count + top_offset) as i32, last_bottom as i32, bottom_count as i32, &mut max_list, inside);
            }
            bottom_count -= 1;
        } else {
            let last_top = top_count - 1;
            let point = ((start[1] as i32 + direction[1] * top_count as i32).rem_euclid(top_size as i32)) as usize;
            let last_point = ((start[1] as i32 + direction[1] * last_top as i32).rem_euclid(top_size as i32)) as usize;
            let bottom_point = ((start[0] as i32 + direction[0] * bottom_count as i32).rem_euclid(bottom_size as i32)) as usize;
            if !((top.flags & ICONT_OPEN != 0 && top_skip == Some(last_top))
                || istore_point_is_gap(&top.store, if direction[1] > 0 { last_point as i32 } else { point as i32 }) != 0
                || outside_mesh_limits(&top.pts[last_point], &bottom.pts[bottom_point], &top.pts[point])) {
                chunk_add_triangle(&mut mesh, (last_top + top_offset) as i32, bottom_count as i32, (top_count + top_offset) as i32, &mut max_list, inside);
            }
            top_count -= 1;
        }
    }
    if !mesh.list.is_empty() { chunk_mesh_add_index(&mut mesh, IMOD_MESH_ENDPOLY, &mut max_list); }
...
pub fn imod_meshes_delete_res(meshes: &mut Vec<crate::imod::libimod::imodel::Imesh>, resolution: i32) -> i32 {
    use crate::imod::libimod::imesh::imesh_resol;
    if meshes.is_empty() { return -1; }
...
pub fn imesh_dup_marked_conts(
    ...
    for index in 0..object.cont.len() {
        if flag != 0 && object.cont[index].flags & flag == 0 { continue; }
        if flag != 0 { object.cont[index].flags &= !flag; }
        if object.cont[index].pts.is_empty() { continue; }
        ...
        if new_index < 0 || istore_copy_cont_surf_items(
            &object.store, &mut output.store, index as i32, new_index, 0,
        ) != 0 { return None; }
    }
    if istore_copy_non_index(&object.store, &mut output.store) != 0 { return None; }
    for surface in 0..=maximum_surface {
        if istore_copy_cont_surf_items(&object.store, &mut output.store, surface, surface, 1) != 0 { return None; }
    }
    Some(output)
}
...
pub fn resection_object(
    ...
    if minimum_z == DEFAULT_VALUE && maximum_z == DEFAULT_VALUE && increment_z == 1 { return 0; }
    let mut index = 0;
    while index < object.cont.len() {
        ...
        if remove { object.cont.remove(index); } else { index += 1; }
    }
    0
}
...
pub fn reduce_object_contours(object: &mut crate::imod::libimod::imodel::Iobj, distance: f32) -> i32 {
    ...
    for contour in &mut object.cont {
        ...
        if count > 4 && !loopback {
            ...
            while tolerance > 0.01 * distance {
                let Some(mut duplicate) = imod_contour_dup(contour) else { return 1; };
                imod_contour_reduce(Some(&mut duplicate), tolerance);
                if duplicate.pts.len() < 4 { tolerance *= 0.5; }
                else { *contour = duplicate; break; }
            }
        }
    }
    0
...
pub fn clean_zero_coordinates(object: &mut crate::imod::libimod::imodel::Iobj) {
    for contour in &mut object.cont {
        for point in &mut contour.pts {
            if point.x < 0.001 && point.x > -0.001 { point.x = 0.; }
            if point.y < 0.001 && point.y > -0.001 { point.y = 0.; }
            if point.z < 0.001 && point.z > -0.001 { point.z = 0.; }
        }
    }
}
...
pub fn extend_open_ends(object: &mut crate::imod::libimod::imodel::Iobj) -> i32 {
    ...
    use crate::imod::libimod::istore::{istore_insert, istore_point_is_gap, GEN_STORE_GAP, GEN_STORE_ONEPOINT, Istore, StoreUnion};

    let z_values: Vec<i32> = object.cont.iter().map(|contour| imod_contour_z_value(Some(contour))).collect();
    ...
    for (index, contour) in object.cont.iter().enumerate() {
        if contour.flags & ICONT_OPEN != 0 && contour.pts.len() > 1 {
            ...
            if (start_gap && end_gap)
                ...
                || (end_gap && istore_point_is_gap(&contour.store, contour.pts.len() as i32 - 3) != 0) {
                phantom.push(index);
            }
        }
    }
    if phantom.is_empty() { return 0; }
    for index in 0..object.cont.len() {
        let contour = &object.cont[index];
        let eligible = contour.flags & ICONT_OPEN != 0 && contour.pts.len() > 1
        ...
        let near_index = *phantom.iter().min_by_key(|&&candidate| (z_values[candidate] - z_values[index]).abs()).unwrap();
        let near = object.cont[near_index].clone();
        let mut start = -1;
        while istore_point_is_gap(&near.store, start + 1) != 0 && start < near.pts.len() as i32 / 2 { start += 1; }
        let mut end = near.pts.len() as i32;
        while istore_point_is_gap(&near.store, end - 2) != 0 && end > near.pts.len() as i32 / 2 { end -= 1; }
        ...
        for point in 0..=start {
            ...
            if imod_point_add(contour, Some(added), point) == 0
                || istore_insert(&mut contour.store, Istore {
                    type_: GEN_STORE_GAP, flags: GEN_STORE_ONEPOINT, index: StoreUnion::from_i(point), value: StoreUnion::default(),
                }) != 0 { return 1; }
        }
        for point in end as usize..near.pts.len() {
            ...
            if imod_point_append(contour, added) == 0
                || istore_insert(&mut contour.store, Istore {
                    type_: GEN_STORE_GAP, flags: GEN_STORE_ONEPOINT,
                    index: StoreUnion::from_i(contour.pts.len() as i32 - 2), value: StoreUnion::default(),
                }) != 0 { return 1; }
        }
    }
    0
...
pub fn imesh_prep_contours(
    ...
    for contour in &mut object.cont {
        if contour.pts.is_empty() { return 1; }
        let mut z = contour.pts[0].z;
        if use_mean_z {
            for point in contour.pts.iter().skip(1) { z += point.z; }
            z /= contour.pts.len() as f32;
        }
        for point in contour.pts.iter_mut().skip(1) { point.z = z; }
    }
    if iobj_close(object.flags) != 0 && extend_open_ends(object) != 0 { return 1; }
    if resection_object(object, minimum_z, maximum_z, increment_z) != 0 { return 1; }
    if reduce_object_contours(object, tolerance) != 0 { return 1; }
...
pub fn next_mesh_z(z_list: &[i32], current_z: i32) -> i32 {
    assert!(!z_list.is_empty(), "native getnextz requires a nonempty Z list");
    for index in 0..z_list.len() - 1 {
        if current_z == z_list[index] { return z_list[index + 1]; }
    }
...
pub fn segment_separation(lower_one: f32, upper_one: f32, lower_two: f32, upper_two: f32) -> f32 {
    ...
    if upper_two - lower_two < minimum_length as f32 { minimum_length = (upper_two - lower_two) as i32; }
    if (lower_one >= lower_two && upper_one <= upper_two) || (lower_two >= lower_one && upper_two <= upper_one) { return 0.; }
    if lower_one > upper_two { return (minimum_length as f32 + lower_one - upper_two); }
    if lower_two > upper_one { return (minimum_length as f32 + lower_two - upper_one); }
    if upper_one > upper_two {
        minimum_length = (upper_one - upper_two) as i32;
        if lower_one - lower_two < minimum_length as f32 { minimum_length = (lower_one - lower_two) as i32; }
    } else {
        minimum_length = (upper_two - upper_one) as i32;
        if lower_two - lower_one < minimum_length as f32 { minimum_length = (lower_two - lower_one) as i32; }
    }
...
pub fn add_mesh_to_object(
    ...
    use crate::imod::libimod::imesh::{imodel_mesh_add, IMESH_MK_SURF, IMESH_MK_TIME};
    mesh.surf = if flags & IMESH_MK_SURF != 0 { contour.surf as i16 } else { 0 };
    mesh.time = if flags & IMESH_MK_TIME != 0 { contour.time as i16 } else { 0 };
...
pub fn backoff_overlap(
    ...
    let Some(mut first_out) = (0..count).find(|&index| imod_point_inside_cont(second, &first.pts[index]) == 0) else { return; };
    let mut last_out = first_out;
    let mut first_in = last_out + 1;
    let mut turns = 0;
    while turns < count {
        if first_in == count { first_in = 0; }
        if imod_point_inside_cont(second, &first.pts[first_in]) != 0 { break; }
        last_out = first_in;
        first_in += 1;
        turns += 1;
    }
    if turns == count { return; }
    let mut before = None;
    for tenth in (2..=9).rev() {
        ...
        if imod_point_inside_cont(second, &point) == 0 {
            before = Some(interpolate_point(first.pts[last_out], first.pts[first_in], 0.1 * (tenth - 1) as f32));
            break;
        }
    }
    ...
    turns = 0;
    while turns < count {
        if first_out == count { first_out = 0; }
        if imod_point_inside_cont(second, &first.pts[first_out]) == 0 { break; }
        last_in = first_out;
        first_out += 1;
        turns += 1;
    }
    ...
    for tenth in (2..=9).rev() {
        ...
        if imod_point_inside_cont(second, &point) == 0 {
            after = Some(interpolate_point(first.pts[first_out], first.pts[last_in], 0.1 * (tenth - 1) as f32));
            break;
        }
    }
    let mut insertion = last_out + 1;
    if let Some(point) = before { imod_point_add(first, Some(point), insertion as i32); insertion += 1; }
    if let Some(point) = after { imod_point_add(first, Some(point), insertion as i32); }
    let mut index = 0;
    while index < first.pts.len() {
        if imod_point_inside_cont(second, &first.pts[index]) != 0 { imod_point_delete(first, index as i32); }
        else { index += 1; }
    }
}
...
pub fn eliminate_overlap(
    ...
    for pass in 0..2 {
        if imodel_contour_overlap(first, second) == 0 { return pass; }
...
pub fn contour_convex_if_simple(contour: Option<&crate::imod::libimod::imodel::Icont>) -> i32 {
    let Some(contour) = contour else { return -1; };
    ...
    for index in 0..contour.pts.len() {
        ...
        let cross = (contour.pts[next].x - contour.pts[index].x) * (contour.pts[second].y - contour.pts[next].y)
            - (contour.pts[next].y - contour.pts[index].y) * (contour.pts[second].x - contour.pts[next].x);
        if index == 0 { all_positive = cross > 0.; }
        if (cross > 0. && !all_positive) || (cross < 0. && all_positive) { return 0; }
    }
    1
}
...
pub fn concavity_area_fraction(
    ...
    let Some(contour) = contour else { return (-1., 0.); };
    if contour.pts.len() < 4 { return (0., 0.); }
    let area = |points: &[crate::imod::libimod::imodel::Ipoint]| -> f32 {
        0.5 * points.iter().enumerate().map(|(index, point)| {
    ...
    for point in points.iter().copied() {
        while hull.len() >= 2 && cross(hull[hull.len() - 2], hull[hull.len() - 1], point) <= 0. { hull.pop(); }
        hull.push(point);
    }
    let lower_len = hull.len();
    for point in points.iter().rev().copied().skip(1) {
        while hull.len() > lower_len && cross(hull[hull.len() - 2], hull[hull.len() - 1], point) <= 0. { hull.pop(); }
        hull.push(point);
    }
    if hull.len() > 1 { hull.pop(); }
    if hull.len() < 3 { return (-1., 0.); }
    let hull_area = area(&hull);
    let difference = hull_area - contour_area;
    (if hull_area > 0. { difference / hull_area } else { 0. }, difference)
}
...
pub fn contour_segment_min_max(
    contour: Option<&crate::imod::libimod::imodel::Icont>, start: usize, end: usize,
) -> Option<(f32, f32, f32, f32)> {
    let contour = contour?;
    if contour.pts.is_empty() || start >= contour.pts.len() || end >= contour.pts.len() { return None; }
    let mut index = start;
    ...
    loop {
        ...
        xmin = xmin.min(point.x); xmax = xmax.max(point.x);
        ymin = ymin.min(point.y); ymax = ymax.max(point.y);
        if index == end { break; }
...
pub fn segment_crosses_contour(
    contour: Option<&crate::imod::libimod::imodel::Icont>,
    x_start: f32, y_start: f32, x_end: f32, y_end: f32, skip_one: usize, skip_two: usize,
) -> i32 {
    let Some(contour) = contour else { return 0; };
    if contour.pts.is_empty() { return 0; }
    let dx_one = x_end - x_start;
    let dy_one = y_end - y_start;
    for point in 0..contour.pts.len() {
        let next = (point + 1) % contour.pts.len();
        if point == skip_one || point == skip_two || next == skip_one || next == skip_two { continue; }
        ...
        if (denominator < 0. && t_numerator <= 0. && u_numerator <= 0.
            && t_numerator >= denominator && u_numerator >= denominator)
            || (denominator >= 0. && t_numerator >= 0. && u_numerator >= 0.
                && t_numerator <= denominator && u_numerator <= denominator) {
            return 1;
        }
    }
...
pub fn check_legal_joiner(
    ...
    if length > 1.0e-5 {
        ...
        if segment_crosses_contour(Some(contour_one), start_x, start_y, end_x, end_y, usize::MAX, usize::MAX) != 0
            || segment_crosses_contour(Some(contour_two), start_x, start_y, end_x, end_y, usize::MAX, usize::MAX) != 0 {
            return 0;
        }
    }
    let min_fraction = if length > 1.0e-5 { (CHECK_MIN_DISTANCE / length).max(CHECK_MIN_FRACTION) } else { CHECK_MIN_FRACTION }.min(0.5);
...
pub fn scan_points_to_segments(
    ...
    other_same_count: usize,
    other: Option<&crate::imod::libimod::imodel::Icont>,
) -> (Option<ContourSegmentCandidate>, Option<ContourSegmentCandidate>) {
    if contour_one.pts.is_empty() || contour_two.pts.is_empty() { return (None, None); }
    ...
    for (point_index, point) in contour_one.pts.iter().copied().enumerate() {
        for segment_index in 0..contour_two.pts.len() {
            ...
            let t = if dx != 0. || dy != 0. {
                (((point.x - start.x) * dx + (point.y - start.y) * dy) / (dx * dx + dy * dy)).clamp(0., 1.)
            } else { 0. };
            let close = crate::imod::libimod::imodel::Ipoint {
                x: start.x + t * dx, y: start.y + t * dy, z: start.z,
            };
            let distance_squared = (close.x - point.x).powi(2) + (close.y - point.y).powi(2);
            let candidate = ContourSegmentCandidate { point_index, segment_index, t, close, distance_squared };
            if fallback.is_none_or(|best| distance_squared < best.distance_squared) { fallback = Some(candidate); }
            let permitted = !check_legality || check_legal_joiner(
                point, close, contour_one, contour_two, same_level, object,
                other_indices, other_same_count, other,
            ) != 0;
            if permitted && legal.is_none_or(|best| distance_squared < best.distance_squared) { legal = Some(candidate); }
        }
    }
    (legal, fallback)
}
...
pub fn find_closest_contour(
    ...
    #[derive(Clone, Copy)]
    struct Pair<'a> { position: usize, index: Option<usize>, contour: &'a crate::imod::libimod::imodel::Icont, same_level: bool }
    let mut choices = Vec::new();
    if let Some(contour) = one_contour {
        choices.push(Pair { position: 1, index: None, contour, same_level: true });
    } else {
        for (position, &raw_index) in list.iter().enumerate().skip(1) {
            if used.get(position).copied().unwrap_or(false) { continue; }
            let Ok(index) = usize::try_from(raw_index) else { continue; };
            let Some(contour) = object.cont.get(index) else { continue; };
            choices.push(Pair { position, index: Some(index), contour, same_level: position < same_level_count });
        }
    }
    let mut unrestricted: Option<ClosestContourMatch> = None;
    let mut designated: Option<ClosestContourMatch> = None;
    for choice in &choices {
        if let Some(connector) = make_connectors(target, choice.contour, true, 0).and_then(|mut items| items.drain(..).next()) {
            ...
            designated = Some(ClosestContourMatch { list_position: choice.position, contour_index: choice.index,
                first_contour_point: true, point_one_index, point_two_index, t: 0., close, distance_squared: 0. });
            break;
        }
        let (forward, _) = scan_points_to_segments(target, choice.contour, false, false, object, other_indices, other_same_count, other);
        if let Some(candidate) = forward {
            let value = ClosestContourMatch { list_position: choice.position, contour_index: choice.index,
                first_contour_point: true, point_one_index: candidate.point_index, point_two_index: candidate.segment_index,
                t: candidate.t, close: candidate.close, distance_squared: candidate.distance_squared };
            if unrestricted.is_none_or(|best| value.distance_squared < best.distance_squared) { unrestricted = Some(value); }
        }
        let (reverse, _) = scan_points_to_segments(choice.contour, target, false, false, object, other_indices, other_same_count, other);
        if let Some(candidate) = reverse {
            let value = ClosestContourMatch { list_position: choice.position, contour_index: choice.index,
                first_contour_point: false, point_one_index: candidate.segment_index, point_two_index: candidate.point_index,
                t: candidate.t, close: candidate.close, distance_squared: candidate.distance_squared };
            if unrestricted.is_none_or(|best| value.distance_squared < best.distance_squared) { unrestricted = Some(value); }
        }
    }
    if designated.is_some() { unrestricted = designated; }
    let nearest = unrestricted?;
    let selected = choices.iter().find(|choice| choice.position == nearest.list_position)?;
    let target_point = if nearest.first_contour_point { target.pts.get(nearest.point_one_index)? } else { &nearest.close };
    let joined_point = if nearest.first_contour_point { &nearest.close } else { selected.contour.pts.get(nearest.point_two_index)? };
    if check_legal_joiner(*target_point, *joined_point, target, selected.contour, selected.same_level,
        object, other_indices, other_same_count, other) != 0 { return Some(nearest); }

    let mut legal: Option<ClosestContourMatch> = None;
    for choice in &choices {
        let (forward, _) = scan_points_to_segments(target, choice.contour, true, choice.same_level, object, other_indices, other_same_count, other);
        if let Some(candidate) = forward {
            let value = ClosestContourMatch { list_position: choice.position, contour_index: choice.index,
                first_contour_point: true, point_one_index: candidate.point_index, point_two_index: candidate.segment_index,
                t: candidate.t, close: candidate.close, distance_squared: candidate.distance_squared };
            if legal.is_none_or(|best| value.distance_squared < best.distance_squared) { legal = Some(value); }
        }
        let (reverse, _) = scan_points_to_segments(choice.contour, target, true, choice.same_level, object, other_indices, other_same_count, other);
        if let Some(candidate) = reverse {
            let value = ClosestContourMatch { list_position: choice.position, contour_index: choice.index,
                first_contour_point: false, point_one_index: candidate.segment_index, point_two_index: candidate.point_index,
                t: candidate.t, close: candidate.close, distance_squared: candidate.distance_squared };
...
pub fn join_all_contours(
    ...
    if list.len() < 2 { return Some(joined); }
    let mut used = vec![false; list.len()];
    for _ in 1..list.len() {
        let mut selected = find_closest_contour(&joined, None, object, list, &used,
            other_indices, other_same_count, other_for_checks, same_level_count)?;
        let object_index = selected.contour_index?;
        let mut next = object.cont.get(object_index)?.clone();
        if selected.list_position < same_level_count && eliminate_overlap(&mut joined, &mut next) != 0 {
            let selected_position = selected.list_position;
            let mut projection = find_closest_contour(&joined, Some(&next), object, list, &used,
                other_indices, other_same_count, other_for_checks, same_level_count)?;
        ...
        if selected.t > 0. && selected.t < 1. {
            if selected.first_contour_point {
                ...
                if imod_point_add(&mut next, Some(selected.close), insertion as i32) == 0 { return None; }
            } else {
                let insertion = selected.point_one_index.checked_add(1)?;
                if imod_point_add(&mut joined, Some(selected.close), insertion as i32) == 0 { return None; }
            }
        }
        let counter_direction = i32::from(selected.list_position >= same_level_count);
        joined = imod_contour_join(Some(&mut joined), Some(&mut next), selected.point_one_index as i32,
            selected.point_two_index as i32, fill, counter_direction)?;
        ...
        while second_index + 1 < second.pts.len() {
            ...
            if first_start.y == second_start_point.y {
                if first_start.x < second_end_point.x && second_start_point.x < first_end.x {
                    if first_start.x < second_start_point.x && first_end.x <= second_end_point.x {
                    ...
                    } else if first_start.x >= second_start_point.x && first_end.x > second_end_point.x {
                        ...
                        let _ = imod_point_add(first, Some(second_start_point), (first_index + 1) as i32);
                        let _ = imod_point_add(first, Some(second_end_point), (first_index + 2) as i32);
                    }
                }
...
pub fn break_contour_inout(
    ...
    second_point: i32,
    fill: i32,
) -> Option<(crate::imod::libimod::imodel::Icont, crate::imod::libimod::imodel::Icont)> {
    use crate::imod::libimod::imodel::{Icont, ICONT_OPEN};
    ...
    if first_point >= contour.pts.len() || second_point >= contour.pts.len() { return None; }
    ...
    if fill == 1 { midpoint.z += 0.75; }
    if fill == -1 { midpoint.z -= 0.75; }
    let mut first = Icont::default();
    let mut second = Icont::default();
    if contour.flags & ICONT_OPEN != 0 { first.flags |= ICONT_OPEN; }
    first.pts.extend_from_slice(&contour.pts[..=first_point]);
    if fill != 0 { first.pts.push(midpoint); }
    first.pts.extend_from_slice(&contour.pts[second_point..]);
    if fill != 0 { second.pts.push(midpoint); }
    second.pts.extend_from_slice(&contour.pts[first_point..=second_point]);
    let size = contour.pts.len() as i32;
    ...
    if istore_extract_changes(&contour.store, &mut first.store, 0, first_point, 0, size) != 0
        || (fill != 0 && istore_extract_changes(&contour.store, &mut first.store, first_point, first_point, first_point + 1, size) != 0)
        || istore_extract_changes(&contour.store, &mut first.store, second_point, size - 1, first_point + if fill != 0 { 2 } else { 1 }, size) != 0
        || (fill != 0 && istore_extract_changes(&contour.store, &mut second.store, first_point, first_point, 0, size) != 0)
        || istore_extract_changes(&contour.store, &mut second.store, first_point, second_point, if fill != 0 { 1 } else { 0 }, size) != 0 {
        return None;
    }
    if reversed { Some((second, first)) } else { Some((first, second)) }
}
...
pub fn smooth_reduce_skeleton(contour: Option<&mut crate::imod::libimod::imodel::Icont>) -> i32 {
    ...
    let mut base = 0usize;
    while base + 1 < count {
        if paired[base] >= 0 { base += 1; continue; }
        let mut found = count;
        ...
        if found < count {
            ...
            while base + 1 < found.saturating_sub(1)
                && paired[base + 1] < 0 && paired[found - 1] < 0
    ...
    while base + 1 < count {
        ...
        if end_break == count { end_break = count - 1; }
        let mut segment_start = base + 1;
        let mut segment_end = end_break - 1;
        if paired[base] == -1 && paired[segment_start] == ((base + count - 1) % count) as isize { segment_start -= 1; }
        if paired[end_break] == -1 && paired[segment_end] == (end_break + 1) as isize { segment_end += 1; }
        let segment_size = segment_end + 1 - segment_start;
        if segment_size < 3 { base += 1; continue; }
        if segment_size >= 4 {
            for fit_start in 0..=segment_size.saturating_sub(5) {
                ...
                ls_fit2(&x, &xsquared, &yrot, fit_size as i32, &mut a, &mut b, Some(&mut c));
                let mut fill_start = fit_size / 2;
                let mut fill_end = fill_start;
                if fit_start == 0 { fill_start = 1; }
                if fit_end == segment_size - 1 { fill_end = fit_size - 2; }
                for fit_index in fill_start..=fill_end {
                    ...
                    if let Ok(pair) = usize::try_from(paired[index]) { contour.pts[pair] = contour.pts[index]; }
                }
            }
        }
        let mut reduced = crate::imod::libimod::imodel::Icont {
            pts: contour.pts[segment_start..=segment_end].to_vec(), ..Default::default()
        };
        ...
        for index in segment_start + 1..segment_end {
            ...
            if let Ok(pair) = usize::try_from(paired[index]) { contour.pts[pair] = replacement; }
        }
        for index in base + 1..end_break {
            if let Ok(pair) = usize::try_from(paired[index]) { paired[pair] = -2; }
            paired[index] = -2;
        }
        base = segment_end;
    }
...
pub fn evaluate_break(
    ...
    orphan_bounds: &[(crate::imod::libimod::imodel::Ipoint, crate::imod::libimod::imodel::Ipoint)],
    first_point: usize,
    second_point: usize,
    current_minimum: f32,
    path_length: f32,
    ...
    use crate::imod::libimod::icont::{imod_contour_area, imod_contour_get_bbox, imodel_contour_scan, imodel_overlap_fractions};
    const INVALID: f32 = 1.0e20;
    const FAILURE: f32 = 1.0e30;
    let (Some(start), Some(end)) = (outer.pts.get(first_point), outer.pts.get(second_point)) else { return (FAILURE, 0.); };
    let distance = ((start.x - end.x).powi(2) + (start.y - end.y).powi(2) + (start.z - end.z).powi(2)).sqrt();
    let maximum_area = (distance + path_length).powi(2) / 12.5664;
    let mut ratio = if maximum_area > 0. { distance / maximum_area.powi(2) } else { INVALID };
    if (!area_only && ratio > current_minimum) || (area_only && maximum_area < current_minimum) {
        return (ratio, maximum_area);
    }
    if segment_crosses_contour(Some(outer), start.x, start.y, end.x, end.y, first_point, second_point) != 0 {
        return (INVALID, 0.);
    }
    let Some((first, second)) = break_contour_inout(Some(outer), first_point as i32, second_point as i32, -join_direction) else {
        return (FAILURE, 0.);
    };
    ...
    ratio = if inner_area > 0. { distance / inner_area.powi(2) } else { INVALID };
    if (!area_only && ratio > current_minimum) || (area_only && inner_area < current_minimum) {
        return (ratio, inner_area);
    }
    let Some(mut inner_scan) = imodel_contour_scan(Some(&inner)) else { return (FAILURE, 0.); };
    ...
    for &index in just_inside {
        if used.get(index).copied().unwrap_or(false) { continue; }
        let (Some(scan), Some(&(minimum, maximum))) = (orphan_scans.get_mut(index), orphan_bounds.get(index)) else { continue; };
        let (mut first_fraction, mut second_fraction) = (0., 0.);
        imodel_overlap_fractions(scan, minimum, maximum, &mut inner_scan, inner_minimum, inner_maximum,
            &mut first_fraction, &mut second_fraction);
        area_sum += second_fraction * inner_area;
    }
    ratio = if area_sum > 0. { distance / area_sum.powi(2) } else { INVALID };
    (ratio, area_sum)
}
...
pub fn add_whole_nest(
    ...
    let Some(mut nest) = nests.get(nest_index) else { return; };
    for &outside in &nest.outside {
        let Some(&raw_index) = usize::try_from(outside).ok().and_then(|index| nest_indices.get(index)) else { continue; };
        let Some(candidate) = usize::try_from(raw_index).ok().and_then(|index| nests.get(index)) else { continue; };
    ...
    let mut append = |contour_index: i32| {
        let Some(contour) = usize::try_from(contour_index).ok().and_then(|index| object.cont.get_mut(index)) else { return; };
    ...
    append(nest.co);
    for &inside in &nest.inside { append(inside); }
}
...
pub fn mesh_unconnected_contours_to_object(
    ...
    if bottom.pts.len() == 1 { bottom.pts.push(bottom.pts[0]); }
    if top.pts.len() == 1 { top.pts.push(top.pts[0]); }
    if object.flags & IMOD_OBJFLAG_OUT != 0 { inside = !inside; }
    let mut direction = [imod_cont_z_direction(Some(&bottom)), imod_cont_z_direction(Some(&top))];
    if direction[0] == 0 { direction[0] = 1; }
    if direction[1] == 0 { direction[1] = 1; }
    let open = crate::imod::libimod::iobj::iobj_close(object.flags) == 0;
    let Some(mesh) = mesh_unconnected_contour_pair(&bottom, &top, scale, inside, direction, [0, 0], open) else { return -1; };
...
pub fn mesh_open_object(
    ...
    mesh_open_object_in_range(
        object, scale, increment_z, flags, skip_passes,
...
pub fn mesh_open_object_in_range(
    ...
    use crate::imod::libimod::icont::{
        imod_contour_get_bbox, imod_contour_make_z_tables, ICONT_CONNECT_BOTTOM,
        ICONT_CONNECT_TOP,
    };
    use crate::imod::libimod::imesh::{DEFAULT_VALUE, IMESH_MK_SKIP, IMESH_MK_SURF, IMESH_MK_TIME};
    use crate::imod::libimod::imodel::{IMOD_OBJFLAG_OUT, ICONT_OPEN};
    ...
    if imod_contour_make_z_tables(
        object, increment_z.max(1), ICONT_CONNECT_TOP | ICONT_CONNECT_BOTTOM,
        &mut contz, &mut zlist, &mut numatz, &mut contatz, &mut zmin, &mut zmax,
        &mut zlsize, &mut nummax,
    ) != 0 || zmin > zmax { return -1; }
    object.mesh.clear();
    ...
    for zpass in 1..=passes {
        for iz in 0..=(zmax - zmin) as usize {
            ...
            if minimum_z != DEFAULT_VALUE && lower_z < minimum_z { continue; }
            if maximum_z != DEFAULT_VALUE && lower_z > maximum_z { continue; }
            let mut upper_z = lower_z;
            for _ in 0..zpass {
                if flags & IMESH_MK_SKIP != 0 {
                    upper_z = get_next_z(&zlist, upper_z);
                    if upper_z > zmax { break; }
                } else { upper_z += increment_z.max(1); }
            }
            if upper_z > zmax { continue; }
            if minimum_z != DEFAULT_VALUE && upper_z < minimum_z { continue; }
            if maximum_z != DEFAULT_VALUE && upper_z > maximum_z { continue; }
            ...
            for (li, &lower_index) in lower.iter().enumerate() {
                ...
                if contz.get(lower_index) != Some(&lower_z) { continue; }
                if object.cont[lower_index].flags & ICONT_CONNECT_TOP != 0 { continue; }
                for (ui, &upper_index) in upper.iter().enumerate() {
                    let upper_index = upper_index as usize;
                    if contz.get(upper_index) != Some(&upper_z) { continue; }
                    if object.cont[upper_index].flags & ICONT_CONNECT_BOTTOM != 0 { continue; }
                    let bottom = &object.cont[lower_index];
                    let top = &object.cont[upper_index];
                    if flags & IMESH_MK_SURF != 0 && bottom.surf != top.surf { continue; }
                    if flags & IMESH_MK_TIME != 0 && bottom.time != top.time { continue; }
                    ...
                    candidates.push((separation, li, ui, lower_index, upper_index,
            ...
            loop {
                let best_regular = candidates.iter().enumerate().filter(|(_, candidate)| {
                    !candidate.5 && !used_lower[candidate.1] && !used_upper[candidate.2]
                }).min_by(|a, b| a.1.0.total_cmp(&b.1.0));
                let best_single = candidates.iter().enumerate().filter(|(_, candidate)| {
                    candidate.5 && !used_lower[candidate.1] && !used_upper[candidate.2]
                }).min_by(|a, b| a.1.0.total_cmp(&b.1.0));
                let Some((_, candidate)) = best_regular.or(best_single) else { break; };
                let (li, ui, lower_index, upper_index) = (candidate.1, candidate.2, candidate.3, candidate.4);
                used_lower[li] = true;
                used_upper[ui] = true;
                let (bottom, top) = (object.cont[lower_index].clone(), object.cont[upper_index].clone());
                object.cont[lower_index].flags |= ICONT_OPEN | ICONT_CONNECT_TOP;
                object.cont[upper_index].flags |= ICONT_OPEN | ICONT_CONNECT_BOTTOM;
                if mesh_unconnected_contours_to_object(object, &bottom, &top, scale, inside, flags) != 0 { return -1; }
            }
        }
    }
...
pub fn mesh_open_tube_object(
    ...
    use crate::imod::libimod::istore::{
        istore_cont_surf_draw_props, istore_default_draw_props, istore_first_change_index,
        istore_next_change, istore_point_is_gap, DrawProps,
    };
    ...
    for contour_index in 0..object.cont.len() {
        ...
        if contour.pts.len() < 2 { continue; }
        let mut contour_props = default_props;
        let (mut contour_state, mut surface_state) = (0, 0);
        let _ = istore_cont_surf_draw_props(
            &object.store, &default_props, &mut contour_props, contour_index as i32,
            contour.surf, &mut contour_state, &mut surface_state,
        );
        ...
        for point_index in 0..contour.pts.len() {
            if point_index as i32 == next_change {
                next_change = istore_next_change(
                    &contour.store, &mut change_cursor, &contour_props, &mut point_props,
                    &mut point_state, &mut point_changes,
                );
            }
            let diameter = if mesh_diameter > 0. { mesh_diameter as f32 }
                else if mesh_diameter == 0. { point_props.linewidth as f32 }
                else if mesh_diameter < -1.0001 { point_props.symsize as f32 }
                else { 2. * imod_point_get_size(object, &contour, point_index as i32) };
            ...
            if make_tube_cont(&mut ring, &contour.pts[point_index], &tangent, scale, diameter, slices) != 0 {
                return -1;
            }
            rings.push(ring);
        ...
        if flags & IMESH_MK_CAP_DOME != 0 && contour_props.no_cap == 0 {
            let diameter = if mesh_diameter > 0. { mesh_diameter as f32 } else { contour_props.linewidth.max(1) as f32 };
            let (start_domes, start_cap) = make_dome_contours(&rings[0], &contour.pts[0], &ring_normals[0], scale, diameter, -1.);
            if let Some(last_dome) = start_domes.last() {
                if let Some(mesh) = make_cap_mesh(last_dome, &start_cap, 1, &default_props, 0, 0) {
                    if add_mesh_to_object(object, &contour, mesh, flags) != 0 { return -1; }
                }
            }
            for dome_index in (0..start_domes.len()).rev() {
                let next = if dome_index == 0 { &rings[0] } else { &start_domes[dome_index - 1] };
                let mut first_props = default_props;
                let mut second_props = default_props;
                if let Some(mesh) = join_tube_cont(&start_domes[dome_index], next, &ring_normals[0],
                    &mut first_props, 0, &mut second_props, 0) {
                    if add_mesh_to_object(object, &contour, mesh, flags) != 0 { return -1; }
                }
            }
        ...
        for point_index in 0..rings.len() - 1 {
            if istore_point_is_gap(&contour.store, point_index as i32) != 0 { continue; }
            let mut tangent = crate::imod::libimod::imodel::Ipoint {
            ...
            let mut first_props = default_props;
            let mut second_props = default_props;
            if let Some(mesh) = join_tube_cont(&rings[point_index], &rings[point_index + 1], &tangent,
                &mut first_props, 0, &mut second_props, 0) {
                if add_mesh_to_object(object, &contour, mesh, flags) != 0 { return -1; }
            }
        }
        if flags & IMESH_MK_CAP_DOME != 0 && contour_props.no_cap == 0 {
            ...
            let diameter = if mesh_diameter > 0. { mesh_diameter as f32 } else { contour_props.linewidth.max(1) as f32 };
            let (end_domes, end_cap) = make_dome_contours(&rings[last], &contour.pts[last], &ring_normals[last], scale, diameter, 1.);
            for dome_index in 0..end_domes.len() {
                let previous = if dome_index == 0 { &rings[last] } else { &end_domes[dome_index - 1] };
                let mut first_props = default_props;
                let mut second_props = default_props;
                if let Some(mesh) = join_tube_cont(previous, &end_domes[dome_index], &ring_normals[last],
                    &mut first_props, 0, &mut second_props, 0) {
                    if add_mesh_to_object(object, &contour, mesh, flags) != 0 { return -1; }
                }
            }
            ...
            let last = contour.pts.len() - 1;
            if let Some(mesh) = make_cap_mesh(&rings[last], &contour.pts[last], 0, &default_props, 0, 0) {
                if add_mesh_to_object(object, &contour, mesh, flags) != 0 { return -1; }
            }
        }
    }
...
pub fn make_dome_contours(
    ...
    diameter: f32,
    direction: f32,
) -> (Vec<crate::imod::libimod::imodel::Icont>, crate::imod::libimod::imodel::Ipoint) {
    ...
    for position in 1..=positions {
        ...
        if position < positions {
            ...
            if make_tube_cont(&mut ring, &cap, normal, scale, 2. * radius, slices) == 0 { rings.push(ring); }
        }
    }
    (rings, cap)
}
...
pub fn skin_unconnected_contour_stack(
    ...
    skin_unconnected_contour_stack_in_range_with_caps(
        object, scale, flags, crate::imod::libimod::imesh::DEFAULT_VALUE,
        crate::imod::libimod::imesh::DEFAULT_VALUE, crate::imod::libimod::imesh::IMESH_CAP_OFF,
    )
}
...
pub fn skin_unconnected_contour_stack_in_range(
    ...
    skin_unconnected_contour_stack_in_range_with_caps(
        object, scale, flags, minimum_z, maximum_z, crate::imod::libimod::imesh::IMESH_CAP_OFF,
    )
}
...
pub fn skin_unconnected_contour_stack_in_range_with_caps_and_skip(
    ...
    let mut contours: Vec<_> = object.cont.iter().filter(|contour| {
        let z = imod_contour_z_value(Some(contour));
        !contour.pts.is_empty()
            && (minimum_z == DEFAULT_VALUE || z >= minimum_z)
            && (maximum_z == DEFAULT_VALUE || z <= maximum_z)
    }).cloned().collect();
    contours.sort_by_key(|contour| imod_contour_z_value(Some(contour)));
    object.mesh.clear();
    if contours.windows(2).any(|pair| imod_contour_z_value(Some(&pair[0])) == imod_contour_z_value(Some(&pair[1]))) {
        return -2;
    }
    for pair in contours.windows(2) {
        let bottom_z = imod_contour_z_value(Some(&pair[0]));
        let top_z = imod_contour_z_value(Some(&pair[1]));
        if flags & IMESH_MK_SURF != 0 && pair[0].surf != pair[1].surf { continue; }
        if flags & IMESH_MK_TIME != 0 && pair[0].time != pair[1].time { continue; }
        if mesh_unconnected_contours_to_object(object, &pair[0], &pair[1], scale, false, flags) != 0 { return -1; }
    }
    if do_cap != crate::imod::libimod::imesh::IMESH_CAP_OFF && !contours.is_empty() {
        ...
        if !skipped(&contours[0]) {
            if let Some(mesh) = contour_cap_mesh(&contours[0], None, -1, cap_inside) {
                if add_mesh_to_object(object, &contours[0], mesh, flags) != 0 { return -1; }
            }
        }
        let last = contours.len() - 1;
        if !skipped(&contours[last]) {
            if let Some(mesh) = contour_cap_mesh(&contours[last], None, 1, cap_inside) {
                if add_mesh_to_object(object, &contours[last], mesh, flags) != 0 { return -1; }
            }
        }
    }
    if flags & crate::imod::libimod::imesh::IMESH_MK_NORM != 0 {
...
pub fn cost_from_area_matrices(
    up: &[f32], down: &[f32], bdim: usize, tdim: usize, bsize: usize,
    sb: usize, st: usize, bmax: usize, tmax: usize, curmin: f64,
) -> (Vec<f64>, Vec<u8>) {
    let stride = bdim + 1;
    ...
    for l in 0..=tmax {
        ...
        for k in 0..=bmax {
            ...
            if k == 0 {
                ...
                if costdown < costup { cost[ind] = costdown; path[ind] = 0; }
                else { cost[ind] = costup; path[ind] = 1; }
                if cost[ind] < rowmin { rowmin = cost[ind]; }
            }
            il = i;
            i = (i + 1) % bdim;
        }
...
pub fn imesh_skin_flags() -> (i32, i32) {
    (SKIN_FLAGS.load(Ordering::Relaxed), FAST_MESH.load(Ordering::Relaxed))
}
...
pub fn invert_connectors(connectors: &mut [Connector], direction: [i32; 2]) {
    ...
    if direction[0] < 0 {
        ...
        for index in 0..connectors.len() {
            (connectors[index].b1, connectors[index].b2) = (connectors[index].b2, connectors[index].b1);
    ...
    if direction[1] < 0 {
        for connector in connectors { (connector.t1, connector.t2) = (connector.t2, connector.t1); }
    }
}
...
pub fn imesh_normal(
    ...
    if distance == 0. { *normal = crate::imod::libimod::imodel::Ipoint { x: 0., y: 0., z: -1. }; }
    else { normal.x /= distance; normal.y /= distance; normal.z /= distance; }
}
...
pub fn imesh_remesh_normal(
    ...
    use crate::imod::libimod::imesh::{
        imesh_resol, IMOD_MESH_BGNPOLY, IMOD_MESH_BGNPOLYNORM, IMOD_MESH_BGNPOLYNORM2,
        IMOD_MESH_END, IMOD_MESH_ENDPOLY, IMESH_FLAG_RES_SHIFT,
    };
    ...
    for mesh in &input {
        if imesh_resol(mesh.flag) != resolution { output.push(mesh.clone()); }
    }
    for (surface, time) in groups {
        let mut triangles = Vec::<[crate::imod::libimod::imodel::Ipoint; 3]>::new();
        let mut unsupported = Vec::new();
        for mesh in input.iter().filter(|mesh| mesh.surf == surface && mesh.time == time && imesh_resol(mesh.flag) == resolution) {
            let mut index = 0;
            let mut found = false;
            while index < mesh.list.len() {
                if mesh.list[index] != IMOD_MESH_BGNPOLY { index += 1; continue; }
                found = true;
                index += 1;
                while index < mesh.list.len() && mesh.list[index] != IMOD_MESH_ENDPOLY {
                    if index + 2 >= mesh.list.len() { break; }
                    let ids = [mesh.list[index], mesh.list[index + 1], mesh.list[index + 2]];
                    if ids.iter().all(|&id| id >= 0 && (id as usize) < mesh.vert.len()) {
                        triangles.push([mesh.vert[ids[0] as usize], mesh.vert[ids[1] as usize], mesh.vert[ids[2] as usize]]);
                    }
                    index += 3;
                }
                index += 1;
            }
            if !found { unsupported.push(mesh.clone()); }
        }
        if triangles.is_empty() { output.extend(unsupported); continue; }
        let mut remesh = crate::imod::libimod::imodel::Imesh {
            surf: surface, time, flag: (resolution as u32) << IMESH_FLAG_RES_SHIFT,
            ..Default::default()
        };
        let new_encoding = imesh_new_poly_norm() != 0;
        remesh.list.push(if new_encoding { IMOD_MESH_BGNPOLYNORM2 } else { IMOD_MESH_BGNPOLYNORM });
        for triangle in triangles {
            let mut ids = [0usize; 3];
            for point in 0..3 {
                ids[point] = remesh.vert.iter().step_by(2).position(|vertex| *vertex == triangle[point])
                    .map_or_else(|| {
            ...
            imesh_normal(&mut normal, &triangle[2], &triangle[0], &triangle[1], Some(scale));
            for id in ids {
                remesh.vert[id + 1].x += normal.x;
                remesh.vert[id + 1].y += normal.y;
                remesh.vert[id + 1].z += normal.z;
                if new_encoding { remesh.list.push(id as i32); }
                else { remesh.list.extend_from_slice(&[(id + 1) as i32, id as i32]); }
            }
        }
        remesh.list.extend_from_slice(&[IMOD_MESH_ENDPOLY, IMOD_MESH_END]);
        output.push(remesh);
...
mod tests {
    ...
    fn remesh_normal_merges_vertices_and_emits_normal_pairs() {
        use crate::imod::libimod::imesh::{IMOD_MESH_BGNPOLY, IMOD_MESH_BGNPOLYNORM2, IMOD_MESH_END, IMOD_MESH_ENDPOLY};
        let triangle = crate::imod::libimod::imodel::Imesh {
            vert: vec![Ipoint::default(), Ipoint { x: 1., y: 0., z: 0. }, Ipoint { x: 0., y: 1., z: 0. }],
            list: vec![IMOD_MESH_BGNPOLY, 0, 1, 2, IMOD_MESH_ENDPOLY, IMOD_MESH_END],
            ..Default::default()
        };
        let mut meshes = vec![triangle.clone(), triangle];
        imesh_set_new_poly_norm(1);
        imesh_remesh_normal(&mut meshes, &Ipoint { x: 1., y: 1., z: 1. }, 0);
    ...
    fn remesh_normal_retains_requested_resolution() {
        use crate::imod::libimod::imesh::{IMOD_MESH_BGNPOLY, IMOD_MESH_END, IMOD_MESH_ENDPOLY, IMESH_FLAG_RES_SHIFT};
        ...
        imesh_remesh_normal(&mut meshes, &Ipoint { x: 1., y: 1., z: 1. }, 1);
    ...
    fn mesh_limits_reject_only_triangles_wholly_outside_xy_bounds() {
        imesh_set_min_max(Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 1., y: 1., z: 0. });
        let outside = Ipoint { x: 2., y: 2., z: 0. };
        assert!(outside_mesh_limits(&outside, &outside, &outside));
        let inside = Ipoint { x: 0.5, y: 0.5, z: 0. };
        assert!(!outside_mesh_limits(&inside, &outside, &outside));
        imesh_set_min_max(Ipoint { x: -1.0e30, y: -1.0e30, z: -1.0e30 }, Ipoint { x: 1.0e30, y: 1.0e30, z: 1.0e30 });
    }
    ...
    fn triangle_append_reverses_inside_winding() {
        ...
        assert_eq!(mesh.list, vec![crate::imod::libimod::imesh::IMOD_MESH_BGNPOLY, 1, 3, 2]);
    }

    #[test]
    fn quick_area_returns_twice_scaled_triangle_area() {
        let origin = Ipoint::default();
        let x = Ipoint { x: 3., y: 0., z: 0. };
        let y = Ipoint { x: 0., y: 4., z: 0. };
    ...
    fn area_matrices_zero_open_contour_wraparound_entries() {
        let bottom = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint::default(), Ipoint { x: 1., y: 0., z: 0. }],
            ..Default::default()
        };
        let top = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0., y: 1., z: 1. }, Ipoint { x: 1., y: 1., z: 1. }],
            ..Default::default()
        };
        let (up, down) = build_area_matrices(
            &bottom, 1, &top, 1, &Ipoint { x: 1., y: 1., z: 1. }, true,
        );
        assert_eq!(up[1], 0.);
        assert_eq!(up[3], 0.);
        assert_eq!(down[2], 0.);
    ...
    fn area_matrices_follow_reversed_contour_direction() {
        let bottom = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 1., y: 0., z: 0. }, Ipoint { x: 0., y: 1., z: 0. }],
            ..Default::default()
        };
        let top = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0., y: 0., z: 1. }, Ipoint { x: 1., y: 0., z: 1. }, Ipoint { x: 0., y: 1., z: 1. }],
            ..Default::default()
        };
        let scale = Ipoint { x: 2., y: 1., z: 1. };
    ...
    fn cap_mesh_encodes_triangles_and_style_at_native_list_offsets() {
        ...
        let props = crate::imod::libimod::istore::DrawProps { red: 1., ..Default::default() };
        let mesh = make_cap_mesh(&contour, &Ipoint { x: 0., y: 0., z: 1. }, 0, &props, 1, 1).unwrap();
        assert_eq!(mesh.vert.len(), 4);
        assert_eq!(mesh.list, vec![
            crate::imod::libimod::imesh::IMOD_MESH_BGNPOLY, 1, 2, 0,
            crate::imod::libimod::imesh::IMOD_MESH_ENDPOLY,
            crate::imod::libimod::imesh::IMOD_MESH_BGNPOLY, 2, 3, 0,
            crate::imod::libimod::imesh::IMOD_MESH_ENDPOLY,
            crate::imod::libimod::imesh::IMOD_MESH_BGNPOLY, 3, 1, 0,
            crate::imod::libimod::imesh::IMOD_MESH_ENDPOLY,
    ...
    fn cap_mesh_omits_triangles_wholly_outside_mesh_bounds() {
        let contour = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 2., y: 2., z: 0. }, Ipoint { x: 3., y: 2., z: 0. }],
            ..Default::default()
        };
        imesh_set_min_max(Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 1., y: 1., z: 1. });
        let mesh = make_cap_mesh(&contour, &Ipoint { x: 2., y: 3., z: 0. }, 1,
                                 &crate::imod::libimod::istore::DrawProps::default(), 0, 0).unwrap();
    ...
    fn tube_ring_has_requested_count_center_and_radius() {
        ...
        assert_eq!(make_tube_cont(
            &mut contour, &Ipoint { x: 4., y: 5., z: 6. },
            &Ipoint { x: 0., y: 0., z: 1. }, &Ipoint { x: 1., y: 1., z: 1. }, 4., 4,
        ), 0);
    ...
    fn tube_join_uses_proportional_triangle_strip_and_syncs_transparency() {
        let first = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0., y: 1., z: 0. }, Ipoint { x: -1., y: 0., z: 0. }, Ipoint { x: 1., y: 0., z: 0. }],
            ..Default::default()
        };
        let second = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0., y: 1., z: 1. }, Ipoint { x: -1., y: 0., z: 1. }, Ipoint { x: 1., y: 0., z: 1. }],
            ..Default::default()
        };
        let mut first_props = crate::imod::libimod::istore::DrawProps { trans: 1, ..Default::default() };
        let mut second_props = crate::imod::libimod::istore::DrawProps::default();
        let mesh = join_tube_cont(&first, &second, &Ipoint { x: 0., y: 0., z: 1. },
                                  &mut first_props, 1 << 2, &mut second_props, 0).unwrap();
        ...
        assert_eq!(mesh.list[19], crate::imod::libimod::imesh::IMOD_MESH_ENDPOLY);
    ...
    fn whole_gap_endpoint_walks_both_sides_without_wrapping_forever() {
        ...
        let contour = crate::imod::libimod::imodel::Icont {
            ...
            store: vec![
                Istore { type_: GEN_STORE_GAP, flags: 0, index: StoreUnion::from_i(1), value: StoreUnion::default() },
                Istore { type_: GEN_STORE_GAP, flags: 0, index: StoreUnion::from_i(2), value: StoreUnion::default() },
                Istore { type_: GEN_STORE_GAP, flags: 0, index: StoreUnion::from_i(3), value: StoreUnion::default() },
            ],
            ..Default::default()
        };
    ...
    fn implicit_connector_inserts_paired_numbered_endpoints_in_direction_order() {
        ...
        let mut bottom = crate::imod::libimod::imodel::Icont { pts: vec![Ipoint::default(); 4], ..Default::default() };
        let mut top = crate::imod::libimod::imodel::Icont { pts: vec![Ipoint::default(); 4], ..Default::default() };
        let mut maximum = 7;
        assert_eq!(add_connector_if_none(&mut bottom, 0, &mut top, 0, &mut maximum, [1, -1]), 0);
        assert_eq!(maximum, 9);
    ...
    fn explicit_connectors_match_numbers_and_expand_gap_third_points() {
        ...
        let connector = |index, number| Istore {
            type_: GEN_STORE_CONNECT, flags: 0, index: StoreUnion::from_i(index), value: StoreUnion::from_i(number),
        };
        let gap = |index| Istore {
            type_: GEN_STORE_GAP, flags: 0, index: StoreUnion::from_i(index), value: StoreUnion::default(),
        };
        let bottom = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint::default(); 4], store: vec![connector(1, 14), gap(1), connector(2, 14)], ..Default::default()
        };
        let top = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint::default(); 4], store: vec![connector(1, 14), gap(1), connector(2, 14)], ..Default::default()
        };
        ...
        assert_eq!((connectors[0].b1, connectors[0].b2, connectors[0].t1, connectors[0].t2), (1, 2, 1, 2));
    ...
    fn same_plane_connector_mode_rejects_multiple_matches() {
        ...
        let bottom = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint::default(); 3], store: vec![connector(0, 1), connector(1, 2)], ..Default::default()
        };
        let top = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint::default(); 3], store: vec![connector(0, 1), connector(1, 2)], ..Default::default()
        };
    ...
    fn cost_backtrace_emits_native_reverse_path_order() {
        ...
        assert_eq!(backtrace_cost_path(&path, 1, 1, 1), vec![CostPathStep::Up, CostPathStep::Down]);
    }
    ...
    fn unconnected_contour_pair_emits_native_marked_triangle_mesh() {
        ...
        let mesh = mesh_unconnected_contour_pair(&bottom, &top, &Ipoint { x: 1., y: 1., z: 1. }, false, [1, 1], [0, 0], false).unwrap();
    ...
    fn gap_management_pairs_open_contour_endpoints_in_closed_object() {
        ...
        let mut bottom = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint::default(), Ipoint { x: 1., y: 0., z: 0. }, Ipoint { x: 2., y: 0., z: 0. }],
            flags: ICONT_OPEN, ..Default::default()
        };
        let mut top = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0., y: 0., z: 1. }, Ipoint { x: 1., y: 0., z: 1. }, Ipoint { x: 2., y: 0., z: 1. }],
            flags: ICONT_OPEN, ..Default::default()
        };
        assert_eq!(manage_gaps(&mut bottom, &mut top, true, [1, 1], &Ipoint { x: 1., y: 1., z: 1. }), 0);
        ...
        assert_eq!(istore_connect_number(&bottom.store, 2), istore_connect_number(&top.store, 2));
    }
    ...
    fn delete_meshes_at_resolution_retains_other_resolution_order() {
        ...
        let mut meshes = vec![
            crate::imod::libimod::imodel::Imesh { flag: 0 << IMESH_FLAG_RES_SHIFT, surf: 1, ..Default::default() },
            crate::imod::libimod::imodel::Imesh { flag: 1 << IMESH_FLAG_RES_SHIFT, surf: 2, ..Default::default() },
            crate::imod::libimod::imodel::Imesh { flag: 0 << IMESH_FLAG_RES_SHIFT, surf: 3, ..Default::default() },
        ];
        assert_eq!(imod_meshes_delete_res(&mut meshes, 0), 0);
        assert_eq!(meshes.iter().map(|mesh| mesh.surf).collect::<Vec<_>>(), vec![2]);
    ...
    fn marked_contour_duplication_consumes_marker_and_omits_mesh_data() {
        ...
        object.cont = vec![
            crate::imod::libimod::imodel::Icont { pts: vec![Ipoint::default()], flags: marker, surf: 2, ..Default::default() },
            crate::imod::libimod::imodel::Icont { pts: vec![Ipoint::default()], surf: 3, ..Default::default() },
            crate::imod::libimod::imodel::Icont { flags: marker, surf: 4, ..Default::default() },
        ];
        object.mesh.push(crate::imod::libimod::imodel::Imesh::default());
    ...
    fn resection_removes_empty_out_of_range_and_off_increment_contours() {
        let contour = |z| crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0., y: 0., z }], ..Default::default()
        };
        let mut object = crate::imod::libimod::imodel::Iobj::default();
        object.cont = vec![contour(0.), contour(1.), contour(2.), contour(3.), crate::imod::libimod::imodel::Icont::default()];
        assert_eq!(resection_object(&mut object, 0, 2, 2), 0);
        assert_eq!(object.cont.iter().map(|contour| contour.pts[0].z).collect::<Vec<_>>(), vec![0., 2.]);
    }

    #[test]
    fn point_reduction_preserves_loopback_contour() {
        let points = vec![
            Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 1., y: 0., z: 0. },
            Ipoint { x: 2., y: 0., z: 0. }, Ipoint { x: 2., y: 0., z: 0. },
            Ipoint { x: 1., y: 0., z: 0. }, Ipoint { x: 0., y: 0., z: 0. },
        ];
        let mut object = crate::imod::libimod::imodel::Iobj::default();
        object.cont.push(crate::imod::libimod::imodel::Icont { pts: points.clone(), ..Default::default() });
    ...
    fn clean_zero_coordinates_only_snaps_strictly_small_components() {
        ...
        object.cont.push(crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0.0009, y: -0.0009, z: 0.001 }], ..Default::default()
        });
        clean_zero_coordinates(&mut object);
        assert_eq!(object.cont[0].pts[0], Ipoint { x: 0., y: 0., z: 0.001 });
    }

    #[test]
    fn open_end_extension_projects_nearby_phantom_gap_to_matching_z() {
        use crate::imod::libimod::imodel::ICONT_OPEN;
        use crate::imod::libimod::istore::{istore_point_is_gap, GEN_STORE_GAP, GEN_STORE_ONEPOINT, Istore, StoreUnion};
        let gap = |index| Istore { type_: GEN_STORE_GAP, flags: GEN_STORE_ONEPOINT, index: StoreUnion::from_i(index), value: StoreUnion::default() };
        let mut object = crate::imod::libimod::imodel::Iobj::default();
        object.cont = vec![
            crate::imod::libimod::imodel::Icont {
                pts: vec![Ipoint { x: -1., y: 0., z: 0. }, Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 2., y: 0., z: 0. }],
                flags: ICONT_OPEN, store: vec![gap(0), gap(1)], ..Default::default()
            },
            crate::imod::libimod::imodel::Icont {
                pts: vec![Ipoint { x: 0., y: 0., z: 1. }, Ipoint { x: 2., y: 0., z: 1. }, Ipoint { x: 4., y: 0., z: 1. }],
                flags: ICONT_OPEN, ..Default::default()
            },
        ];
        assert_eq!(extend_open_ends(&mut object), 0);
        assert_eq!(object.cont[1].pts.len(), 7);
        assert_eq!(object.cont[1].pts[0], Ipoint { x: -1., y: 0., z: 1. });
    ...
    fn contour_preparation_preserves_native_first_point_mean_z_quirk() {
        ...
        let mut object = crate::imod::libimod::imodel::Iobj::default();
        object.cont.push(crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0.0005, y: 0., z: 1. }, Ipoint { x: 1., y: 0., z: 3. }, Ipoint { x: 2., y: 0., z: 5. }],
            ..Default::default()
        });
        assert_eq!(imesh_prep_contours(&mut object, DEFAULT_VALUE, DEFAULT_VALUE, 1, 0., true), 0);
        assert_eq!(object.cont[0].pts[0], Ipoint { x: 0., y: 0., z: 1. });
    ...
    fn mesh_commit_applies_requested_surface_and_time_tags() {
        ...
        let mut object = crate::imod::libimod::imodel::Iobj::default();
        let contour = crate::imod::libimod::imodel::Icont { surf: 7, time: 3, ..Default::default() };
        assert_eq!(add_mesh_to_object(&mut object, &contour, crate::imod::libimod::imodel::Imesh::default(), IMESH_MK_SURF | IMESH_MK_TIME), 0);
        assert_eq!((object.mesh[0].surf, object.mesh[0].time), (7, 3));
    }

    #[test]
    fn interpolation_uses_native_first_to_second_fraction() {
        assert_eq!(interpolate_point(
            Ipoint { x: 2., y: 4., z: 6. }, Ipoint { x: 10., y: 14., z: 18. }, 0.25,
        ), Ipoint { x: 4., y: 6.5, z: 9. });
    }

    #[test]
    fn overlap_elimination_returns_immediately_for_disjoint_contours() {
        let contour = |offset| crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: offset, y: 0., z: 0. }, Ipoint { x: offset + 1., y: 0., z: 0. }, Ipoint { x: offset, y: 1., z: 0. }],
            ..Default::default()
        };
    ...
    fn simple_convexity_uses_consistent_turn_orientation() {
        let contour = |points| crate::imod::libimod::imodel::Icont { pts: points, ..Default::default() };
        let square = contour(vec![Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 1., y: 0., z: 0. }, Ipoint { x: 1., y: 1., z: 0. }, Ipoint { x: 0., y: 1., z: 0. }]);
        let concave = contour(vec![Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 2., y: 0., z: 0. }, Ipoint { x: 1., y: 0.5, z: 0. }, Ipoint { x: 2., y: 2., z: 0. }, Ipoint { x: 0., y: 2., z: 0. }]);
    ...
    fn concavity_fraction_matches_hull_area_difference() {
        let contour = crate::imod::libimod::imodel::Icont {
            pts: vec![
                Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 2., y: 0., z: 0. },
                Ipoint { x: 2., y: 2., z: 0. }, Ipoint { x: 1., y: 1., z: 0. },
                Ipoint { x: 0., y: 2., z: 0. },
            ],
            ..Default::default()
        };
    ...
    fn cyclic_segment_bounds_include_wrapped_endpoints() {
        let contour = crate::imod::libimod::imodel::Icont {
            pts: vec![
                Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 4., y: 1., z: 0. },
                Ipoint { x: 3., y: 5., z: 0. }, Ipoint { x: -2., y: 2., z: 0. },
            ],
            ..Default::default()
        };
        assert_eq!(contour_segment_min_max(Some(&contour), 3, 1), Some((-2., 4., 0., 2.)));
    ...
    fn segment_crossing_skips_incident_contour_edges() {
        let contour = crate::imod::libimod::imodel::Icont {
            pts: vec![
                Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 2., y: 0., z: 0. },
                Ipoint { x: 2., y: 2., z: 0. }, Ipoint { x: 0., y: 2., z: 0. },
            ],
            ..Default::default()
        };
        assert_eq!(segment_crosses_contour(Some(&contour), -1., 1., 3., 1., 4, 5), 1);
        assert_eq!(segment_crosses_contour(Some(&contour), 0., 0., 2., 0., 0, 1), 0);
    }
    ...
    fn legal_joiner_applies_adjacent_slice_nesting_rule() {
        let contour = |xmin: f32, xmax: f32| crate::imod::libimod::imodel::Icont {
            pts: vec![
                Ipoint { x: xmin, y: 0., z: 0. }, Ipoint { x: xmax, y: 0., z: 0. },
                Ipoint { x: xmax, y: 2., z: 0. }, Ipoint { x: xmin, y: 2., z: 0. },
            ],
            ..Default::default()
        };
        ...
        let left = Ipoint { x: 2., y: 1., z: 0. };
        let right = Ipoint { x: 8., y: 1., z: 0. };
        assert_eq!(check_legal_joiner(left, right, &first, &second, true, &object, &[0], 1, None), 1);
        assert_eq!(check_legal_joiner(left, right, &first, &second, false, &object, &[0], 1, None), 0);
        assert_eq!(check_legal_joiner(left, right, &first, &second, true, &object, &[99], 1, None), 0);
    }

    #[test]
    fn contour_segment_scan_reports_closest_projected_connector() {
        let first = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0., y: 0., z: 7. }], ..Default::default()
        };
        let second = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 3., y: -1., z: 4. }, Ipoint { x: 3., y: 1., z: 4. }], ..Default::default()
        };
        let object = crate::imod::libimod::imodel::Iobj::default();
        let (legal, fallback) = scan_points_to_segments(&first, &second, false, false, &object, &[], 0, None);
        ...
        assert_eq!(candidate.close, Ipoint { x: 3., y: 0., z: 4. });
    ...
    fn closest_contour_uses_directional_scan_after_nesting_check() {
        let contour = |xmin: f32, xmax: f32| crate::imod::libimod::imodel::Icont {
            pts: vec![
                Ipoint { x: xmin, y: 0., z: 0. }, Ipoint { x: xmax, y: 0., z: 0. },
                Ipoint { x: xmax, y: 1., z: 0. }, Ipoint { x: xmax, y: 2., z: 0. },
                Ipoint { x: xmin, y: 2., z: 0. },
            ], ..Default::default()
        };
        ...
        let object = crate::imod::libimod::imodel::Iobj::default();
        let matched = find_closest_contour(&target, Some(&joined), &object, &[], &[], &[], 0, Some(&adjacent), 0).unwrap();
        assert_eq!(matched.list_position, 1);
        assert!(matched.first_contour_point);
        assert_eq!(matched.point_one_index, 2);
        assert_eq!(matched.close, Ipoint { x: 8., y: 1., z: 0. });
    }
    ...
    fn join_all_contours_applies_closest_connector_and_join() {
        let triangle = |x: f32| crate::imod::libimod::imodel::Icont {
            pts: vec![
                Ipoint { x, y: 0., z: 0. }, Ipoint { x: x + 1., y: 0., z: 0. },
                Ipoint { x, y: 1., z: 0. },
            ], ..Default::default()
        };
        let mut object = crate::imod::libimod::imodel::Iobj::default();
    ...
    fn scan_contour_subtraction_splits_truncates_and_removes_intervals() {
        let intervals = |values: &[(f32, f32)]| crate::imod::libimod::imodel::Icont {
            pts: values.iter().flat_map(|&(start, end)| [
                Ipoint { x: start, y: 0., z: 0. }, Ipoint { x: end, y: 0., z: 0. },
            ]).collect(), ..Default::default()
        };
        let cutter = intervals(&[(3., 7.)]);
        let mut split = intervals(&[(0., 10.)]);
        subtract_scan_contours(&mut split, &cutter);
        assert_eq!(split.pts.iter().map(|point| point.x).collect::<Vec<_>>(), vec![0., 3., 7., 10.]);

        let mut truncated = intervals(&[(0., 5.)]);
        subtract_scan_contours(&mut truncated, &cutter);
        assert_eq!(truncated.pts.iter().map(|point| point.x).collect::<Vec<_>>(), vec![0., 3.]);
    ...
    fn inout_break_adds_fill_midpoint_and_preserves_return_order() {
        ...
        let contour = crate::imod::libimod::imodel::Icont {
            flags: ICONT_OPEN,
            pts: (0..5).map(|x| Ipoint { x: x as f32, y: 0., z: 2. }).collect(),
            ..Default::default()
        };
        ...
        assert_eq!(first.pts[2], Ipoint { x: 2., y: 0., z: 2.75 });
        assert_eq!(second.pts[0], Ipoint { x: 2., y: 0., z: 2.75 });

        let (reversed_first, reversed_second) = break_contour_inout(Some(&contour), 3, 1, 0).unwrap();
    ...
    fn skeleton_smoothing_handles_paired_pixel_boundary_path() {
        let mut contour = crate::imod::libimod::imodel::Icont {
            pts: vec![
                Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 1., y: 0., z: 0. },
                Ipoint { x: 2., y: 1., z: 0. }, Ipoint { x: 3., y: 0., z: 0. },
                Ipoint { x: 2., y: 1., z: 0. }, Ipoint { x: 1., y: 0., z: 0. },
                Ipoint { x: 0., y: 0., z: 0. },
            ], ..Default::default()
        };
        ...
        let mut short = crate::imod::libimod::imodel::Icont { pts: vec![Ipoint::default(); 4], ..Default::default() };
    ...
    fn break_evaluation_rejects_when_geometric_area_bound_cannot_improve() {
        let outer = crate::imod::libimod::imodel::Icont {
            pts: vec![
                Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 4., y: 0., z: 0. },
                Ipoint { x: 4., y: 4., z: 0. }, Ipoint { x: 0., y: 4., z: 0. },
            ], ..Default::default()
        };
        let (score, inner_area) = evaluate_break(&outer, &[], &[], &mut [], &[], 0, 1,
            1., 0., 16., 1, false);
        assert!(score > 1.);
    ...
    fn whole_nest_selects_level_one_outer_group_once() {
        ...
        let nests = vec![
            Nesting { co: 0, outside: vec![1], ..Default::default() },
            Nesting { co: 1, level: 1, inside: vec![2], ..Default::default() },
        ];
    ...
    fn contour_cap_uses_center_and_native_side_offset() {
        let contour = crate::imod::libimod::imodel::Icont {
            pts: vec![
                Ipoint { x: 0., y: 0., z: 3. }, Ipoint { x: 2., y: 0., z: 3. },
                Ipoint { x: 2., y: 2., z: 3. }, Ipoint { x: 0., y: 2., z: 3. },
            ], ..Default::default()
        };
        let mesh = contour_cap_mesh(&contour, None, -1, false).unwrap();
        assert_eq!(mesh.vert[0], Ipoint { x: 1., y: 1., z: 2.5 });
    ...
    fn simple_stack_caps_propagate_object_inside_out_winding() {
        ...
        let scale = Ipoint { x: 1., y: 1., z: 1. };
        assert_eq!(skin_unconnected_contour_stack_in_range_with_caps(&mut normal, &scale, 0, -999, 999, IMESH_CAP_ALL), 0);
        assert_eq!(skin_unconnected_contour_stack_in_range_with_caps(&mut inside_out, &scale, 0, -999, 999, IMESH_CAP_ALL), 0);
    ...
    fn robust_center_uses_contour_center_of_mass_when_available() {
        let mut contour = crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0., y: 0., z: 2. }, Ipoint { x: 2., y: 0., z: 2. }, Ipoint { x: 2., y: 2., z: 2. }, Ipoint { x: 0., y: 2., z: 2. }],
            ..Default::default()
        };
        let mut center = Ipoint::default();
        assert_eq!(robust_center_of_mass(&mut contour, None, &mut center), 0);
        assert_eq!(center, Ipoint { x: 1., y: 1., z: 2. });
    }
    ...
    fn unconnected_pair_commits_source_format_mesh_to_object() {
        ...
        let mut object = crate::imod::libimod::imodel::Iobj::default();
        assert_eq!(mesh_unconnected_contours_to_object(&mut object, &contour(0., 4), &contour(1., 4), &Ipoint { x: 1., y: 1., z: 1. }, false, IMESH_MK_SURF), 0);
        assert_eq!(object.mesh.len(), 1);
        assert_eq!(object.mesh[0].surf, 4);
        assert_eq!(*object.mesh[0].list.last().unwrap(), crate::imod::libimod::imesh::IMOD_MESH_END);
    }
    ...
    fn simple_stack_skinning_connects_each_neighboring_z_pair() {
        ...
        assert_eq!(skin_unconnected_contour_stack(&mut object, &Ipoint { x: 1., y: 1., z: 1. }, 0), 0);
    ...
    fn simple_stack_defers_multi_contour_section_to_nesting_dispatch() {
        let contour = |z, offset| crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: offset, y: 0., z }, Ipoint { x: offset + 1., y: 0., z }, Ipoint { x: offset, y: 1., z }],
            ..Default::default()
        };
        let mut object = crate::imod::libimod::imodel::Iobj::default();
        object.cont = vec![contour(0., 0.), contour(0., 3.), contour(1., 0.)];
        assert_eq!(skin_unconnected_contour_stack(&mut object, &Ipoint { x: 1., y: 1., z: 1. }, 0), -2);
    ...
    fn simple_stack_range_limits_connected_sections() {
        ...
        assert_eq!(skin_unconnected_contour_stack_in_range(&mut object,
            &Ipoint { x: 1., y: 1., z: 1. }, 0, 1, 2), 0);
        assert_eq!(object.mesh.len(), 1);
    }
    ...
    fn simple_stack_cap_mode_adds_both_endpoint_caps() {
        ...
        assert_eq!(skin_unconnected_contour_stack_in_range_with_caps(&mut object,
            &Ipoint { x: 1., y: 1., z: 1. }, 0, 0, 1,
            crate::imod::libimod::imesh::IMESH_CAP_ALL), 0);
    ...
    fn simple_stack_cap_skip_list_omits_matching_endpoint() {
        ...
        let mut object = crate::imod::libimod::imodel::Iobj::default();
        object.cont = vec![contour(0.), contour(1.)];
        assert_eq!(skin_unconnected_contour_stack_in_range_with_caps_and_skip(&mut object,
            &Ipoint { x: 1., y: 1., z: 1. }, 0, 0, 1,
            crate::imod::libimod::imesh::IMESH_CAP_ALL, Some(&[0])), 0);
        assert_eq!(object.mesh.len(), 2);
    }
    ...
    fn open_surface_pairs_nearest_contours_once_per_section() {
        ...
        let contour = |z, offset| crate::imod::libimod::imodel::Icont {
            pts: vec![
                Ipoint { x: offset, y: 0., z }, Ipoint { x: offset + 1., y: 0., z },
                Ipoint { x: offset, y: 1., z },
            ],
            ..Default::default()
        };
        let mut object = crate::imod::libimod::imodel::Iobj::default();
        object.flags |= IMOD_OBJFLAG_OPEN;
        object.cont = vec![contour(0., 0.), contour(0., 20.), contour(1., 0.2), contour(1., 20.2)];
        assert_eq!(mesh_open_object(&mut object, &Ipoint { x: 1., y: 1., z: 1. }, 1, 0, 1), 0);
    ...
    fn open_surface_range_excludes_connections_outside_bounds() {
        ...
        object.flags |= IMOD_OBJFLAG_OPEN;
        object.cont = vec![contour(0.), contour(1.), contour(2.)];
        assert_eq!(mesh_open_object_in_range(&mut object, &Ipoint { x: 1., y: 1., z: 1. },
            1, 0, 1, 1, 2), 0);
        assert!(!object.mesh.is_empty());
        assert_eq!(object.cont[0].flags & crate::imod::libimod::icont::ICONT_CONNECT_TOP, 0);
    }
    ...
    fn open_tube_builds_rings_joins_and_flat_end_caps() {
        ...
        object.cont.push(crate::imod::libimod::imodel::Icont {
            pts: vec![
                Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 0., y: 0., z: 1. },
                Ipoint { x: 0., y: 0., z: 2. },
            ],
            ..Default::default()
        });
        assert_eq!(mesh_open_tube_object(&mut object, &Ipoint { x: 1., y: 1., z: 1. },
            IMESH_MK_TUBE | IMESH_MK_CAP_TUBE, 24.), 0);
    ...
    fn open_tube_skips_join_at_general_storage_gap() {
        ...
        let mut object = crate::imod::libimod::imodel::Iobj::default();
        object.cont.push(crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 0., y: 0., z: 1. }, Ipoint { x: 0., y: 0., z: 2. }],
            store: vec![Istore { type_: GEN_STORE_GAP, index: StoreUnion::from_i(0), ..Default::default() }],
            ..Default::default()
        });
        assert_eq!(mesh_open_tube_object(&mut object, &Ipoint { x: 1., y: 1., z: 1. }, IMESH_MK_TUBE, 24.), 0);
        assert_eq!(object.mesh.len(), 1);
    }
    ...
    fn open_tube_uses_contour_storage_width() {
        ...
        object.store.push(Istore {
            type_: GEN_STORE_3DWIDTH, index: StoreUnion::from_i(0), value: StoreUnion::from_i(48),
            ..Default::default()
        });
        object.cont.push(crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 0., y: 0., z: 1. }],
            ..Default::default()
        });
        assert_eq!(mesh_open_tube_object(&mut object, &Ipoint { x: 1., y: 1., z: 1. }, IMESH_MK_TUBE, 0.), 0);
        assert_eq!(object.mesh.len(), 1);
    ...
    fn open_tube_applies_point_storage_width_changes() {
        ...
        object.cont.push(crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 0., y: 0., z: 1. }],
            store: vec![Istore { type_: GEN_STORE_3DWIDTH, index: StoreUnion::from_i(1), value: StoreUnion::from_i(48), ..Default::default() }],
            ..Default::default()
        });
        assert_eq!(mesh_open_tube_object(&mut object, &Ipoint { x: 1., y: 1., z: 1. }, IMESH_MK_TUBE, 0.), 0);
    ...
    fn open_tube_contour_no_cap_suppresses_flat_caps() {
        ...
        let mut object = crate::imod::libimod::imodel::Iobj::default();
        object.store.push(Istore { type_: GEN_STORE_NO_CAP, index: StoreUnion::from_i(0), ..Default::default() });
        object.cont.push(crate::imod::libimod::imodel::Icont {
            pts: vec![Ipoint { x: 0., y: 0., z: 0. }, Ipoint { x: 0., y: 0., z: 1. }],
            ..Default::default()
        });
    ...
    fn dome_contours_and_tube_domes_follow_scaled_endpoints() {
        ...
        let ring = crate::imod::libimod::imodel::Icont { pts: vec![Ipoint::default(); 24], ..Default::default() };
        let scale = Ipoint { x: 2., y: 2., z: 2. };
        let (_, cap) = make_dome_contours(&ring, &Ipoint::default(), &Ipoint { x: 0., y: 0., z: 1. },
            &scale, 24., -1.);
        ...
        assert_eq!(mesh_open_tube_object(&mut object, &Ipoint { x: 1., y: 1., z: 1. },
            IMESH_MK_TUBE | IMESH_MK_CAP_DOME, 24.), 0);
    ...
    fn connector_inversion_reverses_bottom_and_swaps_both_endpoints() {
        let mut connectors = vec![
            Connector { b1: 1, b2: 2, t1: 3, t2: 4, ..Default::default() },
            Connector { b1: 5, b2: 6, t1: 7, t2: 8, ..Default::default() },
        ];
        invert_connectors(&mut connectors, [-1, -1]);
        assert_eq!((connectors[0].b1, connectors[0].b2, connectors[0].t1, connectors[0].t2), (6, 5, 8, 7));
    }
}
