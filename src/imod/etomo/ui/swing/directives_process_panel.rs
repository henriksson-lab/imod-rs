...
//! The source unit has no members beyond that inherited state and no GUI
//! override: `showBothAxis()` therefore remains the empty superclass call.
#![allow(dead_code)]
...
/// Java package-private final `DirectivesProcessPanel`, including inherited
/// `AxisProcessPanel` state.
pub struct DirectivesProcessPanel {
    pub axis_process_panel: AxisProcessPanel,
}

impl DirectivesProcessPanel {
    /// Java `DirectivesProcessPanel(BaseManager, InterfaceType, AxisProgressPanel)`.
    pub fn new(
        ...
        instance.axis_process_panel.show_both_axis();
...
mod tests {
    ...
    #[test]
    fn constructor_preserves_directives_superclass_arguments_and_source_call_order() {
        ...
        let panel = DirectivesProcessPanel::new(manager, InterfaceType::DirectiveEditor, progress);
        ...
        assert_eq!(superclass.interface_type, InterfaceType::DirectiveEditor);
