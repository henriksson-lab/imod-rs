...
impl MtfFilterCommand {
    ...
    /// Source plain numeric dose-file formats: 0/1 have image dose only, 2
    /// has `(prior, image)`, and 3 converts `(cumulative, prior)` to image.
    pub fn read_plain_doses(
        &self,
        ...
        if !(0..=3).contains(&dose_type) {
            return Err("plain dose file type must be 0 through 3".into());
        }
        ...
        let rows = text
            ...
            .map(|line| {
                line.split_whitespace()
                    .map(|word| {
                        word.parse::<f32>()
                            .map_err(|_| format!("invalid dose value {word}"))
                    })
        ...
        if rows.len() != sections {
            return Err(format!(
                "dose file has {} rows, expected {sections}",
                rows.len()
            ));
        }
        let mut image = Vec::with_capacity(sections);
        let mut prior = vec![0.0; sections];
        for (index, row) in rows.iter().enumerate() {
            if dose_type < 2 {
                if row.len() != 1 {
                    return Err("dose file requires one value per line".into());
                }
                image.push(row[0]);
            } else {
                if row.len() != 2 {
                    return Err("dose file requires two values per line".into());
                }
                prior[index] = row[0];
                image.push(if dose_type == 3 {
                    row[1] - row[0]
                } else {
                    row[1]
                });
        ...
        if image.iter().any(|dose| *dose < 0.0) {
            return Err("image dose cannot be negative".into());
        }
        if dose_type < 2 {
            crate::imod::libcfshr::extraheader::prior_doses_from_image_doses(
                &image,
                bidirectional,
                &mut prior,
            );
        }
        prior.iter_mut().for_each(|dose| *dose += initial_dose);
        Ok((image, prior))
    }

    /// Source mdoc/HDF-autodoc dose route, backed by the translated autodoc
    /// registry and `getMetadataWeightingDoses` rather than ad-hoc text parsing.
    pub fn read_metadata_doses(
        &self,
        path: impl AsRef<std::path::Path>,
        sections: usize,
        bidirectional: i32,
        ...
        let (mut montage, mut number, mut section_type) = (0, 0, 0);
        let index = crate::imod::libcfshr::autodoc::adoc_open_image_metadata(
            filename.as_bytes(),
            i32::from(append_mdoc),
            &mut montage,
            &mut number,
            &mut section_type,
        );
        if index < 0 {
            return Err(format!(
                "opening metadata dose file {filename}: status {index}"
            ));
        }
        if number as usize != sections {
            crate::imod::libcfshr::autodoc::adoc_done();
            return Err(format!(
                "metadata has {number} sections, expected {sections}"
            ));
        }
        let pieces = (0..sections as i32).collect::<Vec<_>>();
        let mut prior = vec![0.0; sections];
        let mut image = vec![0.0; sections];
        let status = crate::imod::libcfshr::extraheader::get_metadata_weighting_doses(
            index,
            section_type,
            sections as i32,
            &pieces,
        ...
        crate::imod::libcfshr::autodoc::adoc_done();
        if status > 0 {
            return Err("getting doses from metadata".into());
        }
        prior.iter_mut().for_each(|dose| *dose += initial_dose);
        Ok((image, prior))
    }
...
/// Source main's MTF interpolation and inverse/rolloff composition.  Optional
/// `secondary` is the amplifier, regular-radius, or deconvolution table.
pub fn build_inverse_mtf_filter(
    points: &[(f32, f32)],
    nsize: usize,
    delta: f32,
    x_scale: f32,
    ...
    {
        return Err("invalid MTF curve".into());
    }
    let scaled = points
        .iter()
        .map(|&(x, y)| (x * x_scale, y))
        .collect::<Vec<_>>();
    let beta = if inverse_sigma > 1.0e-6 {
        -0.5 / inverse_sigma.powi(2)
    } else {
        0.0
    };
    (0..nsize)
        .map(|index| {
            let frequency = index as f32 * delta;
            let segment = scaled
                .windows(2)
                .position(|pair| frequency <= pair[1].0)
                .unwrap_or(scaled.len() - 2);
            let (x0, y0) = scaled[segment];
            let (x1, y1) = scaled[segment + 1];
            let mtf = (y0 + (y1 - y0) * (frequency - x0) / (x1 - x0)).max(0.0);
            let attenuation = if frequency > inverse_radius {
                (beta * (frequency - inverse_radius).powi(2)).exp()
            } else {
                1.0
            };
            let mut value = if r_weight {
                let radius_scale = delta / 0.5;
                if index == 0 {
                    0.2 * radius_scale
                } else {
                    index as f32 * radius_scale
                }
            } else if mtf < 0.01 {
                1.0
            } else {
                1.0 + attenuation * (maximum_inverse.min(mtf.recip()) - 1.0)
            };
            if let Some((table, table_delta)) = secondary {
                let table_index = (frequency / table_delta) as usize;
                let fraction = frequency / table_delta - table_index as f32;
                let right = *table
                    .get(table_index + 1)
                    .ok_or("secondary filter table is too short")?;
                value *= (1.0 - fraction) * table[table_index] + fraction * right;
            }
            Ok(value * density_scale)
        })
        .collect()
}
...
pub fn taper_pad_slice(
    ...
    if input.len() != nx * ny || nx_pad < nx || ny_pad < ny {
        return Err("invalid taper-padding dimensions".into());
    }
    let mut padded = vec![0.0; (nx_pad + 2) * ny_pad];
...
/// Source dose-weighting setup for individual sections.  Each returned plan
/// can be passed to the ordinary section filter in z order.
pub fn dose_weighted_options(
    section_doses: &[f32],
    prior_doses: &[f32],
    pixel_angstrom: f32,
    a: f32,
    ...
    scale: f32,
    table_size: usize,
) -> Result<Vec<MtfFilterOptions>, String> {
    if section_doses.len() != prior_doses.len() || pixel_angstrom <= 0.0 || table_size < 2 {
        return Err("invalid dose-weighting inputs".into());
    }
    section_doses
        .iter()
        .zip(prior_doses)
        .map(|(&dose, &prior)| {
            let mut ctf = vec![0.0; table_size];
            let mut delta = 0.0;
            crate::imod::libcfshr::filtxcorr::dose_weight_filter(
                prior,
                prior + dose,
                pixel_angstrom,
                a,
            ...
            MtfFilterOptions::new(ctf, delta)
        })
        .collect()
}

/// Source multiple-`FakeSIRTiterations` branch: retain the base MTF table and
/// create one owned output plan per requested nominal iteration count.
pub fn fake_sirt_options(
    base: &MtfFilterOptions,
    iterations: &[i32],
    alpha: f32,
    match_add: f32,
) -> Result<Vec<MtfFilterOptions>, String> {
    if alpha <= 0.0
        || iterations
            .iter()
            .any(|&iteration| !(1..=999).contains(&iteration))
    {
        return Err("fake SIRT iterations must be in 1..=999 and alpha positive".into());
    }
    iterations
        .iter()
        .map(|&iteration| {
            let adjusted = adjusted_fake_iter(iteration as f32);
            let ctf = base
                .ctf
                .iter()
                .enumerate()
                .map(|(index, &value)| {
                    let frequency = index as f32 * base.delta;
                    if frequency >= alpha {
                        value * (1.0 - (1.0 - alpha / frequency).powf(adjusted + match_add))
                    } else {
                        value
                    }
                })
...
/// Owned file-level replacement for the source's input/output-unit lifecycle.
/// The caller selects the source 2-D section loop or the 3-D FFT branch; no
/// global image units, C strings, or manual cleanup are involved.
pub fn run_mtffilter_file(
    input_path: impl AsRef<std::path::Path>,
    output_path: impl AsRef<std::path::Path>,
    options: &MtfFilterOptions,
    filter_3d: bool,
) -> Result<(), String> {
    let mut input = IoMrc::default();
    if !input.read_mrc_file(input_path) {
        return Err("reading MTF input MRC".into());
    }
    let mut output = match input.header.mode {
        4 => filter_packed_fft_mrc(&input, options)?,
        _ if filter_3d => filter_real_mrc_3d(&input, options)?,
        _ => filter_real_mrc_slices(&input, options)?,
    };
    if !output.write_mrc_file(output_path, true) {
        return Err("writing MTF output MRC".into());
    }
    Ok(())
}
