...
/// Loads a piece list named by a native Rust path.
///
/// Matches C `mrc_plist_li(IloadInfo *, MrcHeader *, const char *)`
/// (`plist.c:18`).
pub fn mrc_plist_li(li: &mut LoadInfo, hdata: &MrcHeader, fname: &str) -> i32 {
    if fname.is_empty() {
        return 1;
    }
    let Some(mut fin) = ImodFile::open(fname, "r") else {
        li.plist = 0;
        b3d_error(
    ...
    mrc_plist_load(li, hdata, &mut fin)
}

/// Matches C `mrc_plist_load(IloadInfo *, MrcHeader *, FILE *)` (`plist.c:43`).
pub fn mrc_plist_load(li: &mut LoadInfo, hdata: &MrcHeader, fin: &mut ImodFile) -> i32 {
    plist_load(fin, li, hdata.nx, hdata.ny, hdata.nz)
}

/// Matches C static `plist_load` (`plist.c:48`).
fn plist_load(fin: &mut ImodFile, li: &mut LoadInfo, nx: i32, ny: i32, nz: i32) -> i32 {
    li.plist = nz;
    ...
    for i in 0..nz {
        ...
        for field in 0..3 {
            ...
            let end = token
                .char_indices()
                .position(|(at, c)| !(c.is_ascii_digit() || (at == 0 && (c == '-' || c == '+'))))
                .unwrap_or(token.len());
        ...
        if scanret == 3 {
            let pcoords = li.pcoords.as_mut().unwrap();
    ...
    mrc_plist_proc(li, nx, ny, nz)
}

/// Matches C `mrc_plist_proc(IloadInfo *, int, int, int)` (`plist.c:84`).
pub fn mrc_plist_proc(li: &mut LoadInfo, nx: i32, ny: i32, nz: i32) -> i32 {
    let pcoords = li.pcoords.as_mut().unwrap();
...
pub fn ii_plist_load(filename: &str, li: &mut LoadInfo, nx: i32, ny: i32, nz: i32) -> i32 {
    if filename.is_empty() || nz < 1 || ny < 1 || nx < 1 {
        return 1;
    }
    let Some(mut fin) = ImodFile::open(filename, "r") else {
        return 1;
    };
    ii_plist_load_f(&mut fin, li, nx, ny, nz)
}

/// Matches C `iiPlistLoadF(FILE *, IloadInfo *, int, int, int)` (`plist.c:210`).
pub fn ii_plist_load_f(fin: &mut ImodFile, li: &mut LoadInfo, nx: i32, ny: i32, nz: i32) -> i32 {
    if nz < 1 || ny < 1 || nx < 1 {
        return 1;
    }
    plist_load(fin, li, nx, ny, nz)
}

/// Matches C `iiPlistFromMetadata` (`plist.c:234`).
pub fn ii_plist_from_metadata(
    filename: &str,
    add_mdoc: i32,
    li: &mut LoadInfo,
    nx: i32,
    ...
    ii_plist_from_autodoc(adoc_index, 1, li, nx, ny, nz, montage, num_sect, sect_type)
}

/// Matches C `iiPlistFromAutodoc` (`plist.c:259`).
pub fn ii_plist_from_autodoc(
    adoc_index: i32,
    clear_on_done: i32,
    li: &mut LoadInfo,
    nx: i32,
...
mod tests {
    ...
    fn loads_vendored_piece_coordinate_fixture_and_normalizes_origin() {
        ...
        assert_eq!(ii_plist_load(path, &mut load_info, 100, 80, 9), 0);
    ...
    fn creates_piece_grid_with_source_overlap_clamping() {
        let mut load_info = LoadInfo::default();
        assert_eq!(mrc_plist_create(&mut load_info, 10, 8, 5, 2, 2, 20, 9), 0);
