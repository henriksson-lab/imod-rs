...
/// Makes a square, negative-height bead model in `array`.
pub fn make_model_bead(box_size: i32, bead_size: f32, array: &mut [f32]) {
    assert!(box_size > 0, "a bead model needs a positive side length");
    let side = box_size as usize;
    let model_len = side
        .checked_mul(side)
        .expect("bead model side length overflows its output size");
    assert!(
        array.len() >= model_len,
        "bead model output is smaller than {box_size} by {box_size}"
    );
    ...
    for iy in 0..=icen {
        for ix in 0..=icen {
            ...
            array[ix as usize + iy as usize * side] = height;
            array[side - 1 - ix as usize + (side - 1 - iy as usize) * side] = height;
            array[ix as usize + (side - 1 - iy as usize) * side] = height;
            array[side - 1 - ix as usize + iy as usize * side] = height;
        }
    }
}

/// C `beadIntegral`.
pub fn bead_integral(
    array: &[f32],
    nxdim: i32,
    nx: i32,
    ny: i32,
    ...
    temp: Option<&mut [f32]>,
    ann_pct: f32,
    median: Option<&mut f32>,
) -> f64 {
    let mut temp = temp;
    let mut median = median;
    let Some(required) = usize::try_from(nxdim).ok().and_then(|stride| {
        usize::try_from(ny)
            .ok()
    ...
    for iy in iycen - iradout..=iycen + iradout {
        ...
        for ix in ixcen - idx..=ixcen + idx {
            ...
            let value = array[(ix + iy * nxdim) as usize];
            if radsq <= rcensq {
                ncen += 1;
                censum += value as f64;
            } else if radsq <= routsq && radsq >= rinsq {
                if ann_pct > 0. {
                    let Some(temp) = temp.as_deref_mut() else {
                        return 0.;
                    };
                    if nann as usize >= temp.len() {
                        return 0.;
                    }
                    temp[nann as usize] = value;
                }
                nann += 1;
                annsum += value as f64;
            }
    ...
    *cenmean = (censum / ncen as f64) as f32;
    if ann_pct > 0. {
        let Some(median) = median.as_deref_mut() else {
            return 0.;
        };
        ...
        *median = percentile_float(
            ...
            &mut temp[..nann as usize],
            nann,
        );
    }
    if ann_pct < 0.
        && let Some(median) = median.as_deref_mut()
    {
        *median = ncen as f32;
    }
...
mod tests {
    ...
    fn model_bead_has_source_sign_and_symmetry() {
        ...
        make_model_bead(5, 3., &mut array);
        assert!(array[12] < 0.);
        assert_eq!(array[0], array[24]);
    }

    #[test]
    fn model_bead_only_writes_its_square_output() {
        let mut array = vec![-1.; 13];
        make_model_bead(3, 2., &mut array);
        assert!(array[..9].iter().all(|value| *value <= 0.));
        assert_eq!(&array[9..], &[-1., -1., -1., -1.]);
    }
    #[test]
    fn integral_is_center_minus_annulus() {
        ...
        let mut cen = 0.;
        let mut ann = 0.;
        let value = bead_integral(
            &array, 7, 7, 7, 0.6, 1.0, 2.0, 3.5, 3.5, &mut cen, &mut ann, None, 0., None,
        );
        assert_eq!(cen, 9.);
        assert_eq!(ann, 1.);
        assert_eq!(value, 8.);
