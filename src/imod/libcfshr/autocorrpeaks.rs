...
pub unsafe fn find_auto_corr_peaks(
    array: *mut f32,
    nx_pad: i32,
    ny_pad: i32,
    x_peaks: *mut f32,
    ...
    unsafe {
        ...
        if marked_x == 0. && marked_y == 0. {
            ...
            for index in 0..9_usize {
                if *peak.add(index) > -1e29 {
                    let distance = ((*x_peaks.add(index) * *x_peaks.add(index)
                        + *y_peaks.add(index) * *y_peaks.add(index))
                        as f64)
                        .sqrt();
                    if distance > 1. {
                        count += 1;
            ...
            for index in 0..num_scan {
                if !(*peak.add(index as usize) > -1e29
                    && *x_peaks.add(index as usize) >= 0.
                    && ((*x_peaks.add(index as usize) as f64).abs() > 2.5
                        || (*y_peaks.add(index as usize) as f64).abs() > 2.5))
                {
                    continue;
                }
                isc += 1;
                ...
                let mut ind2 = 0;
                let mut variation = 0.;
                delp_x[scan] = *x_peaks.add(index as usize);
                delp_y[scan] = *y_peaks.add(index as usize);
                indfarp[scan] = index;
                indp[scan] = index;
                find_peak_series(
                    x_peaks,
                ...
                if !no_perp_median {
                    perp_med[scan] = perpendicular_line_median(
                        array,
                        nx_pad,
                        ny_pad,
                        delp_x[scan],
                ...
                for divisor in 2..=((distp[scan] / 3.) as i32) {
                    let (mut try_x, mut try_y) = (
                        *x_peaks.add(index as usize) / divisor as f32,
                        *y_peaks.add(index as usize) / divisor as f32,
                    );
            ...
            ind1 = closest_right_side_peak(
                x_peaks, y_peaks, peak, num_peaks, marked_x, marked_y, -1e29,
            );
            if ind1 >= 0 {
                ind_far[0] = ind1;
                del_x[0] = *x_peaks.add(ind1 as usize);
                del_y[0] = *y_peaks.add(ind1 as usize);
                let mut variation = 0.;
                find_peak_series(
                    x_peaks,
                    y_peaks,
        ...
        *near_ind = ind1;
        ...
        let dist1_zt =
            ((del_x_zt[0] * del_x_zt[0] + del_y_zt[0] * del_y_zt[0]) as f64).sqrt() as f32;
        let min_peak = if num_facs > 1 {
            *peak.add(ind1 as usize) * 0.25
        } else {
            -1e29
        };
        ...
        for ifac in fac_start..num_facs {
            ...
            ind2_for_fac[slot] =
                closest_right_side_peak(x_peaks, y_peaks, peak, num_peaks, vec_x, vec_y, min_peak);
            let ind2 = ind2_for_fac[slot];
            if ind2 < 0 {
                continue;
            }
            vec_x = *x_peaks.add(ind2 as usize);
            vec_y = *y_peaks.add(ind2 as usize);
            if adjust_for_tilt {
                adjust_vector_for_tilt(
                    vec_x, vec_y, tilt_angle, axis_angle, 0, &mut vec_x, &mut vec_y,
                );
            }
            let dist2 = ((*x_peaks.add(ind2 as usize) * *x_peaks.add(ind2 as usize)
                + *y_peaks.add(ind2 as usize) * *y_peaks.add(ind2 as usize))
                as f64)
                .sqrt();
            let mut dist2_zt = ((vec_x * vec_x + vec_y * vec_y) as f64).sqrt() as f32;
            let dot = del_x_zt[0] * vec_x + del_y_zt[0] * vec_y;
            let mut angle_fac =
                (dot / (dist1_zt * dist2_zt)).acos() / (core::f32::consts::PI / 180.);
            ...
            del_x[(1 + ifac) as usize] = *x_peaks.add(ind2 as usize);
            del_y[(1 + ifac) as usize] = *y_peaks.add(ind2 as usize);
            ...
            let len_ratio = (num_for_fac[slot] as f64 * dist2
                / (aspect_ratio as f64 * *num as f64 * dist1)) as f32;
        ...
        *dist2_ptr = dist2_for_fac[ifac];
        ...
        if ifac != 0 {
            *num.add(2) = num_for_fac[2];
            *vectors.add(4) = del_x[3];
            *vectors.add(5) = del_y[3];
        ...
        *vectors = del_x[0];
        *vectors.add(1) = del_y[0];
        *vectors.add(2) = del_x[1];
        *vectors.add(3) = del_y[1];
        0
    }
}

/// `findPeakSeries` (`autocorrpeaks.c:426`).
unsafe fn find_peak_series(
    x: *mut f32,
    y: *mut f32,
    peak: *mut f32,
    n: i32,
        ...
        *near = *far;
        loop {
            ...
            for i in 0..n {
                ...
                let xx = *x.add(i as usize) - ex;
                let yy = *y.add(i as usize) - ey;
            ...
            *count += 1;
            *dx = *x.add(found as usize) / *count as f32;
            *dy = *y.add(found as usize) / *count as f32;
            if *near < 0 {
                *near = *far;
                lo = *peak.add(*far as usize);
                hi = lo;
...
unsafe fn closest_right_side_peak(
    x: *mut f32,
    y: *mut f32,
    p: *mut f32,
    n: i32,
    ...
    unsafe {
        ...
        for i in 0..n {
            if *p.add(i as usize) < minp || *x.add(i as usize) < 0. {
                continue;
            }
            let xx = *x.add(i as usize);
            let yy = *y.add(i as usize);
            if xx * xx + yy * yy < 2. {
                continue;
            }
...
unsafe fn perpendicular_line_median(a: *mut f32, nx: i32, ny: i32, x: f32, y: f32) -> f64 {
    unsafe {
            ...
            ..=((ys.max(y + vy) + max + 2. + 5e-1).floor() as i32)
        {
                ...
                ..=((xs.max(x + vx) + max + 2. + 5e-1).floor() as i32)
            {
                ...
                if xx * xx + yy * yy <= maxsq {
                    ...
                    v.push(*a.add((cx + cy * (nx + 2)) as usize));
                }
            }
        }
        let mut median = -1e37;
...
unsafe fn adjust_vector_for_tilt(
    dx: f32,
    dy: f32,
    tilt: f32,
    axis: f32,
...
mod tests {
    ...
    fn peak_series_refines_a_regular_axis() {
        let mut x = [0., 4., 8., 12., 5.];
        let mut y = [0., 0., 0., 0., 4.];
        let mut peaks = [99., 8., 7., 6., 5.];
    ...
    fn closest_peak_considers_the_mirrored_vector() {
        let mut x = [0., 4., 8., -4.];
        let mut y = [0., 0., 0., 0.];
        let mut peaks = [10., 2., 4., 9.];
        assert_eq!(
            unsafe {
                closest_right_side_peak(
                    x.as_mut_ptr(),
                    y.as_mut_ptr(),
                    peaks.as_mut_ptr(),
                    4,
