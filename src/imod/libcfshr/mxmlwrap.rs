...
mod tests {
    ...
    fn loaded_xml_indexes_elements_values_and_attributes() {
        ...
        assert_eq!(ixml_get_string_value(index, found, &mut value), 0);
        assert_eq!(value, b"3.5");
        assert_eq!(ixml_get_string_value(index, found + 1, &mut value), 0);
        assert_eq!(value, b"7");
        let mut attribute = Vec::new();
        assert_eq!(
            ixml_get_string_attribute(index, found, b"number", &mut attribute),
            0
        );
        assert_eq!(attribute, b"12");
        ixml_clear(index);
    }
}
